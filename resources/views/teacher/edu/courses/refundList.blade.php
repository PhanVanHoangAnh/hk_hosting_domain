            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="dtHorizontalVerticalOrder">
                <thead>
                    <tr class="text-start bg-info text-light fw-bold fs-7 text-uppercase gs-0 bg-light text-nowrap">
                        ...
                        <th sort-by="course_id"
                            sort-direction="{{ $sortColumn == 'course_id' && $sortDirection ? $sortDirection : 'desc' }}"
                            class="min-w-125px text-left fs-8" data-column="course_id">
                        ...
                        <th sort-by="price_train_hours"
                            sort-direction="{{ $sortColumn == 'price_train_hours' && $sortDirection ? $sortDirection : 'desc' }}"
                            class="min-w-100px fs-8 d-none" data-column="price_train_hours">
                            <span class="d-flex align-items-center">
                                <span>
                                ...
                                <span>
                                    <span>
                                        <span
                                            class="material-symbols-rounded ms-2 sort-icon {{ $sortColumn == 'price_train_hours' ? '' : 'text-muted' }}">
                                            sort
                                        </span>
                                    </span>
                                </span>
                        ...
                        <th sort-by="price_refund_hours"
                            sort-direction="{{ $sortColumn == 'price_refund_hours' && $sortDirection ? $sortDirection : 'desc' }}"
                            class="min-w-100px fs-8 d-none" data-column="price_refund_hours">
                            <span class="d-flex align-items-center">
                                <span>
                ...
                <tbody class="text-gray-600">
                    @foreach ($requests as $request)
                        <tr list-control="item">
                            ...
                            <td class="text-left mb-1 text-nowrap" data-filter="mastercard" data-column="course_id">
                                @php
                            ...
                            <td class="text-left mb-1 text-nowrap" data-filter="mastercard" data-column="studied_hours">
                                @php
                                    $groupedCourseStudents = \App\Models\CourseStudent::getCourseStudentsByOrderItemAndStudent($request->order_item_id, $request->student_id)->groupBy('subject_id');
                                @endphp
                            ...
                            <td class="text-left mb-1 text-nowrap" data-filter="mastercard" data-column="remain_hours">
                                {{ number_format($request->orderItem->getTotalMinutes() / 60 - $subjectTotalHoursStudied, 2) }}
                                giờ
                            ...
                            <td class="text-left mb-1 text-nowrap">
                                <div class="d-flex">
                                    <span class="d-inline-block  text-truncate" style="max-width: 150px;"
                                        ...
                                        title="{{ $request->reason }}" data-filter="mastercard"
                                        data-column="reason">
                                        {{ $request->reason }}
                                    </span>
                                </div>
                            ...
                            <td class="text-left mb-1 text-nowrap">
                                <div class="d-flex">
                                    <span class="d-inline-block text-truncate" style="max-width: 150px;"
                                        @if (isset($request->reject_reason)) data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $request->reject_reason }}" @endif
                            ...
                            <td class="text-left">
                                <a href="#" row-action="checkstatus" status-request="{{ $request->status }}"
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                    ...
                                    <!--begin::Menu item-->
                                    <div
                                        class="menu-item px-3 {{ $request->status !== App\Models\RefundRequest::STATUS_PENDING ? 'd-none' : '' }}">
                                        <a row-action="cancel-request" data-action="under-construction"
                                            href="{{ action(
                                                [App\Http\Controllers\Accounting\RefundRequestController::class, 'cancelRequest'],
                                                [
                                                    'id' => $request->id,
                                                ],
                                            ) }}"
                                            class="menu-link px-3">Hủy yêu cầu</a>
                                    </div>
