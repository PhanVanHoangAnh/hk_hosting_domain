...
@section('content')
    <div class="toolbar d-flex flex-stack flex-wrap mb-5 mb-lg-7" id="kt_toolbar">
        ...
        <div class="d-flex align-items-center py-1 d-none">
            <!--begin::Button-->
            <button list-action="create-constract" class="btn btn-flex btn-sm btn-primary fw-bold border-0 fs-6 h-40px" id="buttonCreateConstract">
                ...
                <div class="card-title" list-action="search-action-box">
                    ...
                    <div id="search-form" class="d-flex align-items-center position-relative my-1">
                        ...
                        <input list-action="keyword-input" type="text" data-kt-customer-table-filter="search"
                            class="form-control w-250px ps-12" placeholder="Tìm hợp đồng..." />
                    </div>
                    <!--end::Search-->
                </div>
                <!--begin::Card title-->
                        ...
                        <div class="col-md-3 mb-5">
                            ...
                            <div style="max-height: 200px; overflow-y: auto;">
                                <select list-action="order-type-filter-select" class="form-select filter-select"
                                    data-control="select2" data-close-on-select="false" data-placeholder="Chọn loại dịch vụ"
    ...
    $(() => {
        ...
        // Popup
        OrderRejectPopup.init();
    });

    var OrderRejectPopup = function() {
        var popup;
    ...
    let OrderList = function() {
        ...
        return {
            init: () => {
                list = new DataList({
                    url: "{{ action('\App\Http\Controllers\Accounting\OrderController@list') }}",
    ...
    var CreateReceiptPopup = function() {
            ...
            return {
                init: () => {
                    // Initialize the popup object with the appropriate instance of the Popup component
                    popup = new Popup();
                },
                updateUrl: newUrl => {
                    popup.url = newUrl;
            ...
            init: () => {
                createOrderBtns.forEach(btn => {
                    btn.addEventListener('click', e => {
                        ...
                        pickContactPopup.updateUrl("{{ action('\App\Http\Controllers\Accounting\OrderController@pickContact') }}");
                    });
                })
            }
        }
