    ...
    <form id="CreateReceiptForm-{{ $controlId }}" tabindex="-1" method="post">
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="fv-row mb-7">
                        <!--begin::Label-->
                        <label class="fs-6 fw-semibold mb-2 required">
                            <span class="">Kỳ hạn trả góp</span>
                        </label>

                        <select id="term-select" class="form-select form-control" name="term" data-check-error="{{ $errors->has('term') ? 'error' : 'none' }}"
                            data-control="select2"  data-placeholder="Chọn kỳ hạn trả góp" data-allow-clear="true">
                            <option value="">Chọn kỳ hạn trả góp</option>
                            @foreach(config('term') as $term)
                ...
                <div class="col-md-3">
                    <div class="fv-row mb-7">
                        <!--begin::Label-->
                        <label class="fs-6 fw-semibold mb-2 required">
                            <span class="">Loại thẻ trả góp</span>
                        </label>
                        
                        <select id="installment-card-select" class="form-select form-control" name="installment-card" data-check-error="{{ $errors->has('installment-card') ? 'error' : 'none' }}"
                            data-control="select2"  data-placeholder="Chọn loại thẻ trả góp" data-allow-clear="true">
                            <option value="">Chọn loại thẻ trả góp</option>
                            @foreach(config('installmentCard') as $installmentCard)
                            <option value="{{ $installmentCard }}" >{{ $installmentCard }}</option>
                            @endforeach
                        </select>
                        <x-input-error :messages="$errors->get('installmentCard')" class="mt-2" />
                    </div>
                </div>
                ...
                <div class="col-md-3">
                    <div class="fv-row mb-7">
                        ...
                        <select id="banks-select" class="form-select form-control" name="banks" data-check-error="{{ $errors->has('banks') ? 'error' : 'none' }}"
                            data-control="select2"  data-placeholder="Chọn loại thẻ trả góp" data-allow-clear="true">
                            <option value="">Chọn loại thẻ trả góp</option>
                            @foreach(config('banks') as $banks)
                            <option value="{{ $banks }}" >{{ $banks }}</option>
                            @endforeach
                        </select>
                        <x-input-error :messages="$errors->get('banks')" class="mt-2" />
                    </div>
                </div>
        ...
        <div class="modal-footer flex-center">
            <a row-action="show-link" class="btn btn-primary" id="create-link"
                href="{{ action(
                    [App\Http\Controllers\Accounting\OnePayController::class, 'showLink'],
                    [
                        'id' => $order->id,
                        'amount' => '__AMOUNT__',
                    ]
