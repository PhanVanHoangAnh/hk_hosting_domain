...
<body>
    ...
    <table class="info-table">
        ...
        <tr>
            <th>Địa điểm phụ trách</th>
            <th>Loại chương trình</th>
            <th>Tên chương trình</th>
            <th>Giá gốc chương trình</th>
        </tr>
        @foreach($order->orderItems()->get() as $orderItem)
        <tr>
            <td>
                {!! $orderItem->extracurricular->account ?  trans('messages.branch.' . $orderItem->extracurricular->account->branch)  : '--' !!}
            </td>
            <td>{{$orderItem->extracurricular->type}}</td>
            <td>{{$orderItem->extracurricular->name}}</td> 
            <td>{{ number_format($orderItem->extracurricular->price, 0, '.', ',') }}</td>
        </tr>
        @endforeach
