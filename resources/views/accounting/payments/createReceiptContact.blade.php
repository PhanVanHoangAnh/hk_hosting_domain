                ...
                <div class="col-md-6">
                    <div class="fv-row mb-7">
                        ...
                        <input type="text" class="form-control required" id="price-input" name="amount" data-check-error="{{ $errors->has('amount') ? 'error' : '' }}"
                            placeholder="Số tiền" value="{{ max(0, $order->getTotal() - $order->sumAmountPaid()) }}"/>
    ...
    <script>
        ...
        var CreateReceipt = function() {
            ...
            submit = () => {
                ...
                $.ajax({
                    url: "{{ action('\App\Http\Controllers\Accounting\PaymentRecordController@storeReceiptContact', ['id' => 'contactId']) }}",
                    method: "POST",
                    data: data
                }).done(response => {
