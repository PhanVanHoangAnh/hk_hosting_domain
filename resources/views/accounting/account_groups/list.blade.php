            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
                <thead>
                    <tr class="text-start bg-info text-light fw-bold fs-7 text-uppercase gs-0 text-nowrap">
                        ...
                        @if (in_array('manager_id', $columns) || in_array('all', $columns))
                        <th list-action="sort" sort-by="manager_id"
                            sort-direction="{{ $sortColumn == 'manager_id' && $sortDirection ? $sortDirection : 'desc' }}"
                            class="min-w-125px text-left" data-column="manager_id">
                            <span class="d-flex align-items-center">
                                <span>
                                    Người quản lý
                                </span>
                                <span>
                                    <span>
                                        <span
                                            class="material-symbols-rounded ms-2 sort-icon {{ $sortColumn == 'manager_id' ? '' : 'text-muted' }}">
                                            sort
                                        </span>
                                    </span>
                                </span>
                ...
                <tbody class="text-gray-600">
                    @foreach ($accountGroups as $accountGroup)
                        <tr list-control="item">
                            ...
                            @if (in_array('manager_id', $columns) || in_array('all', $columns))
                                <td data-column="manager_id" class="text-left" data-filter="mastercard">
                                    {{ $accountGroup->manager->name ?? '--'}}
                                </td>
                            @endif
                            ...
                            <td data-column="action">
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                    ...
                                    <div class="menu-item px-3">
                                        <a row-action="update"
                                            href="{{ action(
                                                [App\Http\Controllers\Accounting\AccountGroupController::class, 'edit'],
                                                [
                                                    'id' => $accountGroup->id,
                                                ],
                                            ) }}"
                                            class="menu-link px-3">Cập nhật tài khoản thanh toán</a>
                                    </div>
                                    <!--end::Menu item-->
                                    <!--begin::Menu item-->
    ...
    @else
        <div class="py-15">
            ...
            <p class="fs-4 text-center mb-5">
                Chưa có thông tin nhóm tài khoản!
            </p>
