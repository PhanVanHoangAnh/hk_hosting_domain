                    ...
                    <div class="scrollable-table-excel">
                        <table id="kt_datatable_complex_header" class="table  table-bordered table-sm "
                            ...
                            <thead>
                                ...
                                <tr
                                    class="fw-bold px-4 text-start text-gray-800 fs-7 text-uppercase gs-0 table-warning text-nowrap">
                                    <th colspan="2" class="text-center border">Đăng kí</th>
                                    <th colspan="2" class="text-center border">Đã học</th>
                                </tr>
                                <tr
                                    class="fw-bold px-4 text-start text-gray-800 fs-7 text-uppercase gs-0 table-warning text-nowrap">
                                    <th class="border ps-5 min-w-10px w-auto fw-bold ">Giờ học</th>
                                    <th class="border ps-5 min-w-150px w-auto fw-bold ">Giá trị</th>
                            ...
                            <tbody>
                                @if($orderItem->num_of_vn_teacher_sections)
                                    <tr>
                                        ...
                                        <td class="px-5 ">
                                            {{ number_format($orderItem->getPriceVnTeacherHour() * ($orderItem->getTotalVnMinutes()/60)) }} ₫
                                        </td>
                                        <td class="px-5 "></td>
                                        <td class="px-5 "></td>
                                    </tr>
                                ...
                                @if($orderItem->num_of_foreign_teacher_sections)
                                    <tr>
                                        ...
                                        <td class="px-5 ">
                                            {{ number_format($orderItem->getPriceForeignTeacherHour() * ($orderItem->getTotalForeignMinutes()/60)) }} ₫
                                        </td>
                                        <td class="px-5 "></td>
                                        <td class="px-5 "></td>
                                    </tr>
                                ...
                                @if($orderItem->num_of_tutor_sections)
                                    <tr>
                                        ...
                                        <td class="px-5 ">
                                            {{ number_format($orderItem->getPriceTutorHour() * ($orderItem->getTotalTutorMinutes()/60)) }} ₫
                                        </td>
                                        <td class="px-5 "></td>
                                        <td class="px-5 "></td>
                                    </tr>
                                ...
                                @foreach($studentSections as $studentSection) 
                                    @if($studentSection->section->is_vn_teacher_check)
                                        <tr class="border">
                                            ...
                                            <td class="px-5 ">
                                                {{-- {{ $studentSection->section->vnTeacher->getAmountForSubject($orderItem->subject_id) ? number_format($studentSection->section->calculateInMinutesVnTeacherInSection()/60 * $studentSection->section->vnTeacher->getAmountForSubject($orderItem->subject_id)) : 'Chưa có rate' }}                                                 --}}
                                                
                                                {{ $studentSection->status === App\Models\StudentSection::STATUS_PRESENT ? number_format($studentSection->calculateValueSectionByOrderItem($orderItem->id, 'vn')) . ' ₫' : '0 ₫' }}
                                            </td>
                                        </tr>
                                    @endif
                                    @if($studentSection->section->is_foreign_teacher_check)
                                        <tr class="border">
                                            ...
                                            <td class="px-5 ">
                                                {{-- {{ $studentSection->section->foreignTeacher->getAmountForSubject($orderItem->subject_id) ? number_format($studentSection->section->calculateInMinutesForeignTeacherInSection()/60 * $studentSection->section->foreignTeacher->getAmountForSubject($orderItem->subject_id)) : 'Chưa có rate' }} --}}
                                                
                                                {{ $studentSection->status === App\Models\StudentSection::STATUS_PRESENT ? number_format($studentSection->calculateValueSectionByOrderItem($orderItem->id, 'foreign')) . ' ₫' : '0 ₫' }}
                                            </td>
                                        </tr>
                                    @endif
                                    @if($studentSection->section->is_tutor_check)
                                        <tr class="border">
                                            ...
                                            <td class="px-5 ">
                                                {{-- {{ $studentSection->section->tutor->getAmountForSubject($orderItem->subject_id) ? number_format($studentSection->section->calculateInMinutesTutorInSection()/60 * $studentSection->section->tutor->getAmountForSubject($orderItem->subject_id)) : 'Chưa có rate' }} --}}
                                                {{ $studentSection->status === App\Models\StudentSection::STATUS_PRESENT ? number_format($studentSection->calculateValueSectionByOrderItem($orderItem->id, 'tutor')) . ' ₫' : '0 ₫' }}
