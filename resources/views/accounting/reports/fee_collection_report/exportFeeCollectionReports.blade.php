...
@section('title')
    Cấu hình dữ liệu báo cáo thu phí
@endsection

@section('content')
    <form id="filterContactForm" action="{{ action('App\Http\Controllers\Accounting\Report\FeeCollectionReportController@exportRun') }}"
        method="POST">
        @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#exportContactFilterButton').click(function(event) {
                ...
                var updatedFrom = $('[list-action="updated_at-select"] input[name="updated_at_from"]')
                    .val();
                ...
                $.ajax({
                    url: "{{ action('App\Http\Controllers\Accounting\Report\FeeCollectionReportController@exportRun') }}",
                    type: 'POST',
                    data: formData,
                    beforeSend: function() {
                    ...
                    success: function(response) {
                        // Download the file
                        window.location =
                            "{{ action('App\Http\Controllers\Accounting\Report\FeeCollectionReportController@exportDownload') }}?file=" +
                            response.file;
