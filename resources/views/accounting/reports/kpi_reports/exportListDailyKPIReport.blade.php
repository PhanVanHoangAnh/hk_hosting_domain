...
@section('title')
    Cấu hình dữ liệu báo cáo tình trạng yêu cầu hợp đồng
@endsection

@section('content')
    <form id="filterContactForm" action="{{ action('App\Http\Controllers\Accounting\Report\KPIReportController@exportRun') }}"
        method="POST">
        @csrf
    ...
    <script>
        ...
        $(document).ready(function() {
             ...
             $('#exportContactFilterButton').click(function(event) {
                 ...
                 var updatedFrom = $('[list-action="updated_at-select"] input[name="updated_from"]').val();
                 ...
                 var formData = {
                     ...
                     start_at: updatedFrom,
                     end_at: updatedTo
                 };

                 $.ajax({
                     url: "{{ action('App\Http\Controllers\Accounting\Report\KPIReportController@exportRun') }}",
                     type: 'POST',
                     data: formData,
                     beforeSend: function() {
                     ...
                     success: function(response) {
                         // Download the file
                         window.location = "{{ action('App\Http\Controllers\Accounting\Report\KPIReportController@exportDownload') }}?file=" + response.file;
