...
@section('content')
    <form id="{{ $formId }}" >
        ...
        <div class="scroll-y pe-7  py-10 px-lg-17" >
            ...
            <input type="hidden" name="order_id" value="{{$refundRequest->orderItem->order_id}}">
            <input type="hidden" name="order_item_id" value="{{$refundRequest->orderItem->id}}">
            <input type="hidden" name="method" value="{{ App\Models\PaymentRecord::METHOD_BANK_TRANSFER}}">
            <!--begin::Input group-->
                ...
                <table class="table table-row-bordered table-hover table-bordered">
                    ...
                    <tbody>
                        <tr request-control="row" data-bs-toggle="tooltip" data-bs-trigger="hover"
                            ...
                            <td class="bg-warning fw-bold fs-4">
                ...
                <table class="table table-row-bordered table-hover table-bordered">
                    <thead>
                        <tr class="fw-semibold fs-6 text-gray-800 border-bottom-2 border-gray-200 bg-info">
                            ...
                            <th class="text-nowrap text-white">Mã lớp cũ</th>
                    ...
                    <tr request-control="row" data-bs-toggle="tooltip" data-bs-trigger="hover"
                        ...
                        <td>
                            {{$courseStudent->course->import_id}}
                        </td>
                        <td>
            ...
            <div data-control="contact-selector" class="d-flex align-items-end mb-7" style="width:100%">
                <div style="width:100%">
                    ...
                    <select id="payment-account-select"  class="form-select form-control" data-placeholder="Chọn tài khoản"
                        name="payment_account_id" data-control='select2' readonly>
                        @if ($refundRequest->orderItem->orders->salesperson->accountGroup)
                            @foreach( $refundRequest->orderItem->orders->salesperson->accountGroup->paymentAccountEdu()->get() as $paymentAccount)
        ...
        <div class="modal-footer flex-center">
            <!--begin::Button-->
            <button  id="ApprovedAndCreateReceipt" 
                type="submit" class="btn btn-primary me-3 {{ $refundRequest->status != App\Models\RefundRequest::STATUS_PENDING ? 'd-none' : '' }}">
            ...
            <a row-action="reject"
                id="buttonReject" class="btn btn-info me-3  {{ $refundRequest->status != App\Models\RefundRequest::STATUS_PENDING ? 'd-none' : '' }}"> 
                <span class="mx-2">
                    Không duyệt
                </span>
            </a>
