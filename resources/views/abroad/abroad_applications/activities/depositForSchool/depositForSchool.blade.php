<div class="row" id="deposit_for_school">
            ...
            <div class="fw-semibold">
                ...
                <div class="mt-1">
                    ...
                    <button value="deposit_for_school" action-control="deposit_for_school" type="button"
                        class="badge badge-success {{ $abroadApplication->isDoneDepositForSchool() ? 'd-none' : '' }}"
    ...
    <div class="col-md-9" form-data="deposit-school-manager-container"
        data-upload-url="{{ action([App\Http\Controllers\Abroad\AbroadController::class, 'storeDepositForSchool'], ['id' => $abroadApplication->id]) }}"
        data-delete-url="{{ action([App\Http\Controllers\Abroad\AbroadController::class, 'deleteDepositForSchool'], ['id' => $abroadApplication->id]) }}">
        ...
        <form form-data="deposit-for-school-form" method="POST" action="{{ action([App\Http\Controllers\Abroad\AbroadController::class, 'updateDepositForSchool'], ['id' => $abroadApplication->id]) }}" class="row d-flex justify-content-around">
            @csrf
            <div class="col-md-4">
                <input value="{{ isset($abroadApplication->deposit_school_cost) ? $abroadApplication->deposit_school_cost : '' }}" name="deposit_school_cost" class="form-control" type="text" placeholder="Số tiền">
                <x-input-error :messages="$errors->get('deposit_school_cost')" class="mt-2"/>
            </div>
            <div class="col-md-4">
                <div class="form-outline">
                    <div data-control="date-with-clear-button"
                        class="d-flex align-items-center date-with-clear-button">
                        <input data-control="input" name="deposit_school_date" placeholder="=asas" type="date"
                            class="form-control" value="{{ isset($abroadApplication->deposit_school_date) ? $abroadApplication->deposit_school_date : '' }}" placeholder="" />
                        <span data-control="clear" class="material-symbols-rounded clear-button"
                            style="display:none;">close</span>
                    </div>
                </div>
                
                <x-input-error :messages="$errors->get('deposit_school_date')" class="mt-2"/>
            </div>
            <div class="col-md-4">
...
<script>
    ...
    var DepositForSchoolManager = class {
        ...
        getContainerForm() {
            return this.getContainer().find('[form-data="deposit-school-manager-container"]');
        }
        ...
        getUploadUrl() {
            return this.getContainerForm().attr('data-upload-url');
        }

        getDeleteUrl() {
            return this.getContainerForm().attr('data-delete-url');
        }

        getFileElements() {
            return this.getContainerForm().find('[data-control="file"]').toArray();
        }

        getStatusSelect() {
            return this.getContainerForm().find('[name="deposit_school_date"]');
        }

        getCostInput() {
            return this.getForm().find('[name="deposit_school_cost"]');
        }
        ...
        getUpLoadFileBtn() {
            return this.getContainerForm().find('[action-control="upload-file-btn"]');
        }

        getFileInput() {
            return this.getContainerForm().find('[action-control="file-input"]');
        }
        ...
        clickDoneBtn() {
            ...
            $.ajax({
                url: '{{ action([App\Http\Controllers\Abroad\AbroadApplicationStatusController::class, 'updateDoneAbroadApplication'], ['id' => ':id']) }}'
