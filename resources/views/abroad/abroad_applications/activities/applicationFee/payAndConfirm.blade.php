                                    ...
                                    <td class="text-left mb-1 text-nowrap">
                                        @if(isset($applicationSchool->file_fee_paid) )
                                            <div data-file-name="{{ basename($applicationSchool->file_fee_paid) }}" data-control="active-file" class="row my-0">
                                                ...
                                                <div class="col-md-3 ps-0 d-flex justify-content-start align-items-center">
                                                    <button class="btn btn-danger btn-sm delete-btn"
                                                    data-delete-url="{{ action([App\Http\Controllers\Abroad\AbroadController::class, 'deleteFileFeePaid'], ['id' => $applicationSchool->id]) }}"
                                                    class="btn btn-danger btn-sm">Xóa</button>
                                                </div>
                                        ...
                                        @else
                                            <div form-data="fee_paid" data-application-id="{{ $applicationSchool->id }}">
                                                <input type="file" name="file_fee_paid" action-control="file-confirmation-{{$applicationSchool->id}}-input" class="d-none">
                                                <button action-control="upload-file-fee-paid-{{$applicationSchool->id}}-btn" class="fs-6 fw-bold btn btn-info btn-outline-secondary btn-sm fs-4 upload-btn"
                                                        data-upload-url="{{ action([App\Http\Controllers\Abroad\AbroadController::class, 'storeFileFeePaid'], ['id' => $applicationSchool->id]) }}">
                                                    Upload
                                                </button>
            ...
            delete(url) {
                ...
                $.ajax({
                    ...
                    ASTool.alert({
                        ...
                        ok: function() {
                            UpdatePopupPayAngConfirm.getUpdatePopup().load();
                        }
                    });
                }).fail(response => {
                    removeSubmitEffect();
