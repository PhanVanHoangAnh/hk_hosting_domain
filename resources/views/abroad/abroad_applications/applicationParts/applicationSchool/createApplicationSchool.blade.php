...
@section('title')
    Thêm mới trường yêu cầu tuyển sinh
@endsection
@php
    $createApplicationSchool = 'createApplicationSchool_' . uniqid();
@endphp
@section('content')
    <form id="{{ $createApplicationSchool }}"
        action="{{ action(
            [App\Http\Controllers\Abroad\AbroadController::class, 'doneCreateApplicationSchool'],
            [
                'id' => $id,
            ],
        ) }}">
        ...
        pe-7 py-10 px-lg-17" >
                    ...
                    <div class="col-md-4">
                        ...
                        <select list-action="abroad_status" name="type_school" class="form-select filter-select "  data-control="select2"
                            ...
                            @foreach (\App\Models\ApplicationSchool::getAllType() as $type)
                                <option  value="{{ $type }}">{{ trans('messages.application_school.type.' . $type) }}</option>
                            @endforeach
            ...
            <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <div data-control="date-with-clear-button" class="d-flex align-items-center date-with-clear-button">
                            <input data-control="input" value="{{ date('Y-m-d') }}" name="apply_date" 
            ...
            <!--end::Label-->
            <!--begin::Input-->
            <textarea class="form-control d-none" name="requirement" placeholder="Nhập yêu cầu" rows="3" cols="40"></textarea>
            <!--end::Input-->
        ...
        <div class="modal-footer flex-center">
            <!--begin::Button-->
            <button id="CreateApplicationSchoolButton" type="submit" class="btn btn-primary">
    ...
    <script>
        $(() => {
            createApplicationSchool.init();
        })

        var createApplicationSchool = function() {
            let form;
            let submitBtn;
            ...
            submit = () => {
                ...
                $.ajax({
                    ...
                    // success alert
                    CreateApplicationSchoolPopup.getUpdatePopup().hide();
                    ASTool.alert({
            ...
            return {
                init: () => {
                    ...
                    form = document.querySelector('#{{ $createApplicationSchool }}');
                    submitBtn = document.querySelector("#CreateApplicationSchoolButton");
