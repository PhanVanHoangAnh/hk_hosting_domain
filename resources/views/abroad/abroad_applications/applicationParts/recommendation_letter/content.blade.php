...
<div class="row" id="recommendationLetterScreenId">
        ...
        <div class="d-flex">
            ...
            <div class="fw-semibold">
                ...
                <div class="mt-1">
                    ...
                    <button value="recommendation_letters" action-control="recommendation_letters" type="button" class="badge badge-success {{ $abroadApplication->isDoneRecommendationLetter() ? 'd-none' : '' }}" data-bs-toggle="tooltip" data-bs-placement="right" title="Bấm vào đây nếu đã hoàn thành">
                        &#10004;
                    </button>
                </div>
            </div>
    ...
    <div class="col-md-9" form-data="manager-container" 
        data-upload-url="{{ action([App\Http\Controllers\Abroad\AbroadController::class, 'storeRecommendationLetter'], ['id' => $abroadApplication->id]) }}"
        data-delete-url="{{ action([App\Http\Controllers\Abroad\AbroadController::class, 'deleteRecommendationLetter'], ['id' => $abroadApplication->id]) }}">
        ...
        <div class="row d-flex justify-content-around">
            {{-- Date input --}}
            <div class="col-md-4">
                <input type="date" value="{{$dayFinish->recommendation_letters ?? ''}}" name="recommendation_letters" class="form-control">
            </div>
            {{-- Draft form --}}
            <div class="col-md-4">
...
<script>
    ...
    var Manager = class {
        ...
        getForm() {
            return this.getContainer().find('[form-data="manager-container"]');
        }
        ...
        getDraftFileElements() {
            return this.getForm().find('[data-control="draft-file"]').toArray();
        }

        getActiveFileElements() {
            return this.getForm().find('[data-control="active-file"]').toArray();
        }
        ...
        getUpLoadDraftBtn() {
            return this.getForm().find('[action-control="upload-draft-btn"]');
        }

        getUpLoadActiveBtn() {
            return this.getForm().find('[action-control="upload-done-btn"]');
        }
        ...
        getDraftInput() {
            return this.getForm().find('[action-control="draft-input"]');
        }

        getDoneInput() {
            return this.getForm().find('[action-control="done-input"]');
        }
        getDoneBtn() {
