...
@php
    $createSatsCorePopup = 'createSatsCorePopup_' . uniqid();
@endphp

@section('content')
<div>
    <form id="{{ $createSatsCorePopup }}" method="POST" tabindex="-1" enctype="multipart/form-data" 
        action="{{ action([App\Http\Controllers\Abroad\LoTrinhChienLuocController::class, 'doneCreateSatScore'], ['abroadApplicationId' => $abroadApplicationId]) }}">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-lg-4 col-md-6 col-sm-6 col-6 mb-2">
                        <div class="form-outline">
                            ...
                            <input name="sat_score" type="text" value="" class="form-control" placeholder="Điểm thi Sats...">
                            <x-input-error :messages="$errors->get('name')" class="mt-2"/>
                        </div>
                    </div>
...
<script>
    var createSatsScoreManager;

    $(() => {
        createSatsScoreManager = new createSatsScoreManager({
            container: $('#{{ $createSatsCorePopup }}')
        })
    })

    var createSatsScoreManager = class {
        constructor(options) {
