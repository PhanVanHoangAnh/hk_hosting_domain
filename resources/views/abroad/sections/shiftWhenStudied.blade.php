                                            ...
                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                                ...
                                                <div
                                                    ...
                                                    <a data-ids-delete="{{ $section->id }}"
                                                        href="{{ action(
                                                            [App\Http\Controllers\Abroad\SectionReportsController::class, 'create'],
                                                            [
                                                                'id' => $section->id,
                                                ...
                                                <div class="menu-item px-3  ">
                                                    ...
                                                    <a data-ids-delete="{{ $section->id }}"
                                                        href="{{ action(
                                                            [App\Http\Controllers\Abroad\SectionReportsController::class, 'edit'],
                                                            [
                                                                'id' => $section->id,
                                                                'contact_id' => $student->student->id,
                                                            ],
...
<script>
    ...
    var CheckSwitch = class {
        ...
        getStudied() {
            return $('input[value="present"]:checked');
        }
        getExcusedAbsence() {
            return $('input[value="excused_absence"]:checked');
        }
        getUnexcusedAbsence() {
            return $('input[value="unexcused_absence"]');
        }
        getStartAt() {
            return $('input[name="start_at"]');
        }
        getEndAt() {
            return $('input[name="end_at"]');
        }
        getAction() {
        ...
        handleRadioChange() {
            ...
            if (studied.length > 0) {
                const startAtValue =
                    "{{ isset($startAt) ? date('H:i', strtotime($startAt)) : (isset($section) ? date('H:i', strtotime($section->start_at)) : '') }}";
                const endAtValue =
                    "{{ isset($endAt) ? date('H:i', strtotime($endAt)) : (isset($section) ? date('H:i', strtotime($section->end_at)) : '') }}";
