            ...
            <label class="ms-2">
                <p class="fs-2 fw-bold fw-semibold me-8">
                    <input name="checkShift" value="{{ \App\Models\Section::STATUS_STUDIED }}" list-action="check-item"
                        class="form-check-input mt-1 me-2" checked type="radio" {{ $section->status === \App\Models\Section::STATUS_STUDIED ? 'checked' : '' }} />Học
                </p>
            </label>
            <label class="ms-2">
                <p class="fs-2 fw-bold fw-semibold ">
                    <input name="checkShift" value="{{ \App\Models\Section::STATUS_CANCELLED }}" {{ $section->status === \App\Models\Section::STATUS_CANCELLED || $section->status === \App\Models\Section::LATE_CANCELLED_TEACHER || $section->status === \App\Models\Section::LATE_CANCELLED_STUDENT ? 'checked' : '' }} list-action="check-item"
                        class="form-check-input mt-1 me-2" type="radio" />Nghỉ
                </p>
            </label>
        </div>
        <div class="px-lg-17 py-3" id="shiftWhenStudied">
            @include('abroad.sections.shiftWhenStudied')
        </div>

        <!-- Thêm id cho div chứa nội dung khi nghỉ -->

        <div class="px-lg-17 py-3 d-none" id="shiftWhenCancelled">
            @include('abroad.sections.shiftWhenCancelled')
        </div>
        <!--end::Scroll-->
    ...
    <script>
        ...
        $(() => {
            transferClassManager = new TransferManager({
             ...
             // Hàm để kiểm tra trạng thái của radio button và hiển thị nội dung tương ứng
        ...
        function checkRadioButton() {
            let selectedValue = $('input[name="checkShift"]:checked').val();
        ...
        var TransferManager = class {
            ...
            events() {
                ...
                //     }
                // });
                 // Sự kiện thay đổi cho radio buttons
                $('input[name="checkShift"]').change(function() {
                    checkRadioButton();
                });
                ...
                _this.container.addEventListener('submit', function(e) {
                    ...
                    const actionWhenStudied =
                        "{{ action([App\Http\Controllers\Abroad\SectionController::class, 'saveAttendancePopup'], ['id' => $section->id]) }}";
                    const actionWhenCancelled =
                        "{{ action([App\Http\Controllers\Abroad\SectionController::class, 'saveShift'], ['id' => $section->id]) }}";
                    // const url = _this.container.getAttribute('action');
                    const url = _this.getCheckShift() === "{{ \App\Models\Section::STATUS_STUDIED }}" ?
                        actionWhenStudied : actionWhenCancelled;
