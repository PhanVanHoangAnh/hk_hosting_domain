<div class="ms-2">
    <div>
        <p class="fs-2 fw-bold fw-semibold">
            <input name="cancelled" value="{{ \App\Models\Section::STATUS_CANCELLED }}" {{ $section->status === \App\Models\Section::STATUS_CANCELLED ? 'checked' : '' }} list-action="check-item"
        ...
        <p class="fs-2 fw-bold fw-semibold ">
            <input name="cancelled" list-action="check-item" class="form-check-input mt-1 me-2 deselect-on-cancelled"
                value="{{ \App\Models\Section::STATUS_UNPLANNED_CANCELLED }}" {{ $section->status === \App\Models\Section::LATE_CANCELLED_STUDENT || $section->status === \App\Models\Section::LATE_CANCELLED_TEACHER ? 'checked' : '' }} type="radio" />Không kế hoạch
        </p>
    ...
    <div class="ms-8 d-none" id="reasonCancelled">
        <p class="fs-2 fw-bold fw-semibold ">
            <input name="unplannedCancelled" value="{{ \App\Models\Section::LATE_CANCELLED_STUDENT }}" {{ $section->status === \App\Models\Section::LATE_CANCELLED_STUDENT  ? 'checked' : '' }}
        ...
        <p class="fs-2 fw-bold fw-semibold ">
            <input name="unplannedCancelled" list-action="check-item"
                ...
                value="{{ \App\Models\Section::LATE_CANCELLED_TEACHER }}" {{ $section->status === \App\Models\Section::LATE_CANCELLED_TEACHER ? 'checked' : '' }} type="radio" />Do giảng viên (không tính giờ
            giảng
            viên)
        ...
        <!--end::Label-->
        <!--begin::Input-->
        <textarea class="form-control delete-on-note" name="note" placeholder="Nhập ghi chú/lý do!" rows="5" cols="40">{{ $section->note }}</textarea>
...
<script>
    ...
    function checkRadioButtonCancelled() {
            // Lấy giá trị của radio button được chọn
            let cancelledValue = $('input[name="cancelled"]:checked').val();
