...
@section('content')
    ...
    <form id="{{ $formId }}"
        action="{{ action(
            [App\Http\Controllers\Abroad\ContactRequestController::class, 'update'],
            [
    ...
    <script>
        ...
        var ContactRequestsUpdate = function() {
            ...
            function reloadShowView(contactRequestId) {
                var data = $(form).serialize();
                $.ajax({
                    url: "{{ action('App\Http\Controllers\Abroad\ContactRequestController@show', ['id' => $contactRequest->id]) }}",
                    method: 'GET',
                    data: data,
