                ...
                <thead>
                    <tr class="text-start bg-info text-light fw-bold fs-7 text-uppercase gs-0 bg-light text-nowrap">
                        ...
                        @if (in_array('birthday', $columns) || in_array('all', $columns))
                            <th
                                class="min-w-125px text-left bg-info" data-column="birthday">
                                <span class="d-flex align-items-center">
                                    <span>
                                       Ngày sinh
                                    </span>
                        ...
                        @if (in_array('father_phone', $columns) || in_array('father_phone', $columns))
                            <th
                                class="min-w-125px text-left" data-column="father_phone">
                                <span class="d-flex align-items-center">
                                    <span>
                                        Số điện thoại cha
                                    </span>
                        ...
                        @if (in_array('mother_phone', $columns) || in_array('mother_phone', $columns))
                            <th 
                                class="min-w-125px text-left" data-column="mother_phone">
                                <span class="d-flex align-items-center">
                                    <span>
                                        Số điện thoại mẹ
                                    </span>
                        ...
                        @if (in_array('status_abroad', $columns) || in_array('status_abroad', $columns))
                            <th 
                                class="min-w-125px text-left" data-column="status_abroad">
                                <span class="d-flex align-items-center">
                                    <span>
                                        Tình trạng hồ sơ
                                    </span>
                        ...
                        @endif
                        @if ((in_array('account_manager_extracurricular_id', $columns) || in_array('account_manager_extracurricular_id', $columns)))
                            <th 
                                class="min-w-125px text-left" data-column="account_manager_extracurricular_id">
                                <span class="d-flex align-items-center">
                                    <span>
                                        Trưởng nhóm ngoại khóa
                                    </span>
                                </span>
                            </th>
                        @endif
                        ...
                        <tr list-control="item">
                            ...
                            @if (in_array('name', $columns) || in_array('all', $columns))
                                <td data-column="name" data-control="freeze-column" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                                    <a href="{{ action(
                                        [App\Http\Controllers\Abroad\ExtracurricularController::class, 'details'],
                                        [
                                            'id' => $abroadApplication->id,
                                        ],
                                    ) }}"
                                     class="link-primary fw-bold">
                                        {{ $abroadApplication->student ? $abroadApplication->student->name : '--' }}
                                    </a>
                                </td>
                            @endif
                            @if (in_array('birthday', $columns) || in_array('all', $columns))
                            ...
                            @if (in_array('email', $columns) || in_array('all', $columns))
                                <td data-column="email" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                                    ...
                                    @if($abroadApplication->student && $abroadApplication->student->email)
                                        {{ $abroadApplication->student->email }}
                                    @else
                                        --
                                    @endif                                                                    
                                </td>
                            ...
                            @if (in_array('phone', $columns) || in_array('all', $columns))
                                <td data-column="phone" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                                    ...
                                    @if ($abroadApplication->student && $abroadApplication->student->phone)
                                        {{ $abroadApplication->student->phone }}                                        
                                    @else
                                        --
                                    @endif
                                </td>
                            ...
                            @if (in_array('school', $columns) || in_array('all', $columns))
                                <td data-column="school" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                                    ...
                                    @if($abroadApplication->student && $abroadApplication->student->school)
                                        {{ $abroadApplication->student->school }}
                                    @else
                                        --
                                    @endif
                                </td>
                        ...
                        @endif
                            ...
                            @if (in_array('father_id', $columns) || in_array('all', $columns))
                                <td data-column="father_id" class="text-left mb-1 text-nowrap" style=""
                                    ...
                                    {{ isset($abroadApplication->student->father) ? $abroadApplication->student->father->name : '--' }}
                                </td>
                            @endif
                            @if (in_array('father_phone', $columns) || in_array('all', $columns))
                                <td data-column="father_phone" class="text-left mb-1 text-nowrap" style=""
                                    data-filter="mastercard">
                                    {{ isset($abroadApplication->student->father) ? $abroadApplication->student->father->phone : '--' }}
                                </td>
                            @endif
                            ...
                            @if (in_array('mother_id', $columns) || in_array('all', $columns))
                                <td data-column="mother_id" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                                    {{ isset($abroadApplication->student->mother) ? $abroadApplication->student->mother->name : '--' }}
                                </td>
                            @endif
                            @if (in_array('mother_phone', $columns) || in_array('all', $columns))
                                <td data-column="mother_phone" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                                    {{ isset($abroadApplication->student->mother) ? $abroadApplication->student->mother->phone : '--' }}
                                </td>
                            @endif
                            ...
                            @if (in_array('status_abroad', $columns) || in_array('all', $columns))
                                <td data-column="status_abroad" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                            ...
                            @endif
                            @if ( (in_array('account_manager_extracurricular_id', $columns) || in_array('all', $columns)))
                            <td data-column="account_manager_extracurricular_id" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                                {{ $abroadApplication->managerExtracurricular ? $abroadApplication->managerExtracurricular->name : '---'}}
                            </td>
                            @endif
                            ...
                            @if (in_array('updated_at', $columns) || in_array('all', $columns))
                                <td data-column="updated_at" class="text-left mb-1 text-nowrap"
                                    data-filter="mastercard">
                                    {{ $abroadApplication->updated_at->format('d/m/Y') }}
                                </td>
                            @endif
                            ...
                            <td class="text-left">
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                    ...
                                    @if (Auth()->user()->can('handover', $abroadApplication))
                                        <div class="menu-item px-3">
                                            <a  row-action="updateStatusAbroadApplication"
                                                class="menu-link px-3 {{ $status == 'assigned' ? 'd-none' : ''}}"
                                                href="{{ action(
                                                    [App\Http\Controllers\Abroad\ExtracurricularController::class, 'handover'],
                                                    [
                                                        'id' => $abroadApplication->id,
                                                    ],
                                                ) }}">
