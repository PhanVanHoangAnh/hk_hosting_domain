...
<div class="mb-10">
    <div class="form-outline mb-7">
        ...
        <div class="d-flex align-items-center">
            <label for="" class="form-label fw-semibold text-info">{{ $order && $order->type == App\Models\Order::TYPE_REQUEST_DEMO ? 'Người liên hệ' : 'Người ký dịch vụ' }}</label>
        </div>
    ...
    <div data-control="relationship-section">
        <div class="form-outline mb-7">
            <label class="form-check form-check-custom form-check-solid">
                ...
                <span class="form-check-label fw-semibold fs-3 text-info">
                    {{ $order && $order->type == App\Models\Order::TYPE_REQUEST_DEMO ? 'Người liên hệ' : 'Người ký dịch vụ' }} cũng là học viên
                </span>
            </label>
        </div>
        ...
        <div box-not-toggle box-toggle-selector='[name="signer_is_student"]' box-toggle-value="1" style="display:none;">
            ...
            <div box-control="relationship-box" style="display:none;">
                <div class="form-outline mb-7">
                    <label for="" class="form-label fw-semibold text-info">Quan hệ người ký dịch vụ với học viên</label>
        ...
        <script>
            $(function() {
                new RelationshipBox({
                    ...
                    url: '{{ action('\App\Http\Controllers\Abroad\OrderController@relationshipForm') }}',
    ...
    @if (true || !$order->isRequestDemo())
        $(() => {
            ...
            contactRequestManager = new ContactRequestManager({
                url: '{{ action([App\Http\Controllers\Abroad\OrderController::class, 'pickContactRequest']) }}',
