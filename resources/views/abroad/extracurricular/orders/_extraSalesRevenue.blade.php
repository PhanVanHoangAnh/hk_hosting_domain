...
<div class="col-lg-12 col-md-12 col-sm-12 col-12 mb-8 mt-8">
    @php
        $extraItemRevenueSharingUniqId = 'I' . uniqid();
    @endphp

    <div id="{{ $extraItemRevenueSharingUniqId }}">
        <div class="row">
    ...
    @php
        $defaultSale = \App\Models\Account::find(\App\Models\Order::find($order->id)->sale);
        $saleItems = [];
    ...
    <script>
        var RevenueDistribution = class {
            ...
            updateTotalPrice(price) {
                this.totalPrice = this.convertStringPriceToNumber(price);
                this.reset();
            }
            ...
            render() {
                $.ajax({
                    url: "{{ action([App\Http\Controllers\Abroad\RevenueDistributionController::class, 'getSalesRevenuedList']) }}",
                    method: 'post',
                    data: {
        ...
        var Row = class {
            ...
            events() {
                ...
                this.getPriceInput().on('change', (e) => {
                    e.preventDefault();
                ...
                this.getSaleSelector().on('change', (e) => {
                    e.preventDefault();
...
<script>
    ...
    $(() => {
        revenueDistribution = new RevenueDistribution({
            container: () => {
                return document.querySelector('#{{ $extraItemRevenueSharingUniqId }}')
            }
        })

        $('[data-control="price-input"]').on('change', (e) => {
            revenueDistribution.updateTotalPrice(e.target.value);
        })
    }) 
</script>