            ...
            <div class="row">
                <div class="col-md-6" id="paymentAccountContainer-{{ $controlId }}" >
                    <div class="fv-row mb-7">
                        ...
                        <select id="payment-account-select-{{ $controlId }}" class="form-select form-control" data-placeholder="Chọn tài khoản" data-control="select2" name="payment_account_id" required readonly>
                            @if ($order->salesperson->accountGroup)
                                @foreach( $order->salesperson->accountGroup->paymentAccountEdu()->get() as $paymentAccount)
    ...
    <script>
        ...
        var CreateReceipt = function() {
            ...
            submit = () => {
                ...
                $.ajax({
                    url: "{{ action('\App\Http\Controllers\Abroad\PaymentRecordController@storeReceiptContact', ['id' => 'contactId']) }}",
                    method: "POST",
                    data: data
                }).done(response => {
