...
@section('title')
    Danh sách đơn hàng của {{ $contactRequests->first()->contact->name }}
@endsection
            ...
            <div class="card-body pt-0">
                ...
                <div class="table-responsive table-head-sticky freeze-column" style="max-height:calc(100vh - 420px);">
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_contacts_table">
                            <thead>
                                <tr class="text-start bg-info text-light fw-bold fs-7 text-uppercase gs-0 text-nowrap"> 
                                    <th class="min-w-125px ">Salesperson</th>
                                ...
                                @foreach ($contactRequests as $contactRequest)
                                    <tr list-control="item">
                                        <td class="text-left" data-filter="mastercard">
                                            {{ $contactRequest->account ? $contactRequest->account->name : 'Chưa bàn giao' }}

                                        </td>
                                        <td class="text-left">{{ $contactRequest->created_at->format('d/m/Y') }}</td> 
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    {{-- <div class="modal-footer flex-end">
                        <!--begin::Button-->
                        <button type="button" class="buttonCreateNewNoteLog btn btn-primary btn-create-note-log">
                            Thêm ghi chú
                        </button>
                        <!--end::Button-->
                    </div> --}}
