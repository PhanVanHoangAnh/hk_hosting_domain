...
@section('content')
    ...
    <div class="toolbar d-flex flex-stack flex-wrap mb-5 mb-lg-7" id="kt_toolbar">
        ...
        <div class="page-title d-flex flex-column py-1">
            ...
            <h1 class="d-flex align-items-center my-1">
                <span class="text-dark fw-bold fs-1">Danh Sách Tag</span>
            </h1>
            <!--end::Title-->
            <!--begin::Breadcrumb-->
            <ul class="breadcrumb breadcrumb-separatorless fs-7 my-1">
                ...
                <!--end::Item-->
                <!--begin::Item-->
                <li class="breadcrumb-item text-muted">Quản lý Tag</li>
                <!--end::Item-->
                <!--begin::Item-->
                <li class="breadcrumb-item">
                    <span class="bullet bg-gray-200 w-5px h-2px"></span>
                ...
                <!--end::Item-->
                <!--begin::Item-->
                <li class="breadcrumb-item text-dark">Danh sách Tag</li>
                <!--end::Item-->
            </ul>
            <!--end::Breadcrumb-->
        </div>
        ...
        <div class="d-flex align-items-center py-1">
            ...
            <!--begin::Button-->
            <a href="#" class="btn btn-flex btn-sm btn-primary fw-bold border-0 fs-6 h-45px" id="buttonCreateTag">
    ...
    <!--end::Toolbar-->
    <!--begin::Post-->
    <div id="TagsIndexContainer" class="position-relative" id="kt_post">
        <!--begin::Card-->
        <div class="card">
            <!--begin::Card header-->
                        ...
                        <div class="menu-sub menu-sub-dropdown p-3 w-150px">
                            ...
                            <div class="menu-item">
                                <a href="#" class="menu-link px-3"
                                    ...
                                    <span class="menu-title" id="buttonDeleteTags">Xóa tag đã chọn</span>
                                </a>
                            </div>
                            <!--end::Menu item-->
                        </div>
                ...
                <div class="card-title" list-action="search-action-box">
                    ...
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <input list-action="keyword-input" type="text" data-kt-customer-table-filter="search"
                            class="form-control w-250px ps-12" placeholder="Nhập để tìm tag" />
                    </div>
                    <!--end::Search-->
                </div>
    ...
    <script>
        ...
        var DeleteTagsPopup = function() {
            ...
            var showDeleteTags = function() {
                popupDeleteTags.setData({
                    tag_ids: TagsList.getList().getSelectedIds(),
                });
            ...
            return {
                init: function() {
                    popupDeleteTags = new Popup({
                        url: "{{ action('\App\Http\Controllers\Marketing\TagController@deleteTags') }}",
                    });
        ...
        //
        var CreateTagPopup = function() {
            var popupCreateTag;
            var buttonCreateTag;

            // show
            var showTagModal = function() {
                popupCreateTag.load();
            };
            ...
            return {
                init: function() {
                    // create tag popup
                    popupCreateTag = new Popup({
                        url: "{{ action('\App\Http\Controllers\Marketing\TagController@create') }}",
                    });

                    // create tag  button
                    buttonCreateTag = document.getElementById('buttonCreateTag');

                    // nhấp vào nút tạo thẻ
                    buttonCreateTag.addEventListener('click', (e) => {
                        e.preventDefault();
            ...
            return {
                init: () => {
                    $('[list-action="account-select"]').on('change', function() {
                        ...
                        TagsList.getList().setAccountFilter(selectedValues);
                    ...
                    // $('[list-action="account-select"]').on('change', function() {
                    //     const accountSelected = Array.from(this
                    //         .selectedOptions).map(option =>
                    //         option
                    //         .value);
                    //     if ($(this).val() && $(this).val().includes('all')) {
                    //         $(this).find('option').not(':selected').prop('selected',
                    //             true);
                    //     }
                    //     TagsList.getList().setAccountIds(accountSelected);
                    // });

                    // // Press the filter button
                    // document.querySelector("#filter-btn").addEventListener(
                    //     'click', () => {
                    ...
                    $('[list-action="created_at-select"]').on('change', function() {
                        ...
                        TagsList.getList().setCreatedAtFrom(fromDate);
                        TagsList.getList().setCreatedAtTo(toDate);
                        TagsList.getList().load();
                    });
                    ...
                    $('[list-action="updated_at-select"]').on('change', function() {
                        ...
                        // Gửi phạm vi ngày tạo đến ContactsList và tải lại danh sách
                        TagsList.getList().setUpdatedAtFrom(fromDate);
                        TagsList.getList().setUpdatedAtTo(toDate);
                        TagsList.getList().load();
                    });
                }
            };
        ...
        var TagsList = function() {
            ...
            return {
                init: function() {
                    list = new DataList({
                        url: "{{ action('\App\Http\Controllers\Marketing\TagController@list') }}",
                        container: document.querySelector('#TagsIndexContainer'),
                        listContent: document.querySelector('#TagsIndexListContent'),
                    });
                    list.load();
                },
        ...
        var DataList = class {
            ...
            deleteItem(url) {
                // success alert
                ASTool.confirm({
                    message: 'Bạn có chắc muốn xóa Tag',
                    ok: function() {
                        // effect
