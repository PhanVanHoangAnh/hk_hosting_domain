...
<div class="card-body pt-0 mt-5">
    @if ($demands->count())
        <div class="table-responsive table-head-sticky freeze-column" style="max-height:calc(100vh - 420px);">
            <!--begin::Table-->
            {{-- <table class="table table-striped table-bordered table-hover table-header-fixed dataTable no-footer" id="kt_demands_table"> --}}
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_demands_table">
                <thead>
                    <tr class="text-start bg-info text-light fw-bold fs-7 text-uppercase gs-0 text-nowrap">
                        ...
                        @if (in_array('creator_id', $columns) || in_array('all', $columns))
                            <th list-action="sort" sort-by="creator_id" data-control="creator_id"
                                sort-direction="{{ $sortColumn == 'creator_id' && $sortDirection ? $sortDirection : 'desc' }}"
                                class="min-w-100px fs-8 bg-info" data-column="creator_id">
                                <span class="d-flex align-items-center">
                                    <span>
                                        Người tạo
                                    </span>
                                    <span>
                                        <span>
                                            <span
                                                class="material-symbols-rounded ms-2 sort-icon {{ $sortColumn == 'creator_id' ? '' : 'text-muted' }}">
                                                sort
                                            </span>
                                        </span>
                                    </span>
                        ...
                        @if (in_array('status', $columns) || in_array('all', $columns))
                            <th list-action="sort" sort-by="status" data-control="status"
                                sort-direction="{{ $sortColumn == 'status' && $sortDirection ? $sortDirection : 'desc' }}"
                                class="min-w-100px fs-8 bg-info" data-column="status">
                                <span class="d-flex align-items-center">
                                    <span>
                                        Trạng thái
                                    </span>
                ...
                <tbody class="text-gray-600">
                    @foreach ($demands as $demand)
                        <tr list-control="item">
                            <td class="ps-1">
                                <div class="form-check form-check-sm form-check-custom">
                                    <input data-item-id="{{ $demand->id }}" list-action="check-item"
                                        class="form-check-input" type="checkbox" value="{{ $demand->id }}" />
                                </div>
                            </td>
                            @if (in_array('name', $columns) || in_array('all', $columns))
                                <td data-filter="mastercard" data-column="name">
                                    {{ $demand->name }}
                                </td>
                            @endif

                            @if (in_array('creator_id', $columns) || in_array('all', $columns))
                                <td data-filter="mastercard" data-column="creator_id">
                                    {{ $demand->account ? $demand->account->name : '' }}                                
                                </td>
                            @endif
                            ...
                            @if (in_array('status', $columns) || in_array('all', $columns))
                                <td data-filter="mastercard" data-column="status">
                                    @php
                                    $bgs = [
                                        // App\Models\Demand::STATUS_PAUSE => 'secondary text-white',
                                    
                                        App\Models\Demand::STATUS_ACTIVE => 'success',

                                        App\Models\Demand::STATUS_DELETED => 'warning',
                                    ];
                                @endphp
                                <span class="badge bg-{{ $bgs[$demand->status] ?? 'info' }}">
                                    {{ trans('messages.demand.status.'. $demand->status) }}
                                </span>
                                </td>
                            @endif
                            ...
                            @if (in_array('updated_at', $columns) || in_array('all', $columns))
                                <td data-filter="mastercard" data-column="updated_at">
                                    {{ $demand->updated_at->format('d/m/Y') }}
                                </td>
                            @endif
                            ...
                            <td data-column="action">
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                    ...
                                    <div class="menu-item px-3">
                                        <a row-action="update"
                                            href="{{ action(
                                                [App\Http\Controllers\System\DemandController::class, 'edit'],
                                                [
                                                    'id' => $demand->id,
                                                ],
                                            ) }}"
                                    ...
                                    {{-- @if (Auth()->user()->can('delete', $demand)) --}}
                                        <div class="menu-item px-3">
                                            <a row-action="delete"
                                                href="{{ action(
                                                    [App\Http\Controllers\System\DemandController::class, 'destroy'],
                                                    [
                                                        'id' => $demand->id,
                                                    ],
                                                ) }}"
    ...
    @else
        <div class="py-15">
            ...
            <p class="text-center d-none">
                <a class="btn btn-outline btn-outline-default" id="addSalesContactSearch">
                    <span class="d-flex align-items-center">
