...
<div class="card-body pt-0 mt-5">
    ...
    <div id="staff-groups-table-box" class="table-responsive scrollable-orders-table">
        <!--begin::Table-->
        <table class="table align-middle table-row-dashed fs-6 gy-5" id="dtHorizontalVerticalOrder">
            <thead>
                <tr class="text-start bg-info text-light fw-bold fs-7 text-uppercase gs-0 bg-light text-nowrap">
                    ...
                    <th list-action="sort" sort-by="name" sort-direction="{{ $sortColumn == 'name' && $sortDirection ? $sortDirection : 'desc' }}" class="min-w-125px text-left" data-column="name">
                    ...
                    @if(in_array('group_manager_id', $columns) || in_array('group_manager_id', $columns))
                    <th list-action="sort" sort-by="group_manager_id" sort-direction="{{ $sortColumn == 'group_manager_id' && $sortDirection ? $sortDirection : 'desc' }}" class="min-w-125px text-left" data-column="group_manager_id">
                        <span class="d-flex align-items-center">
                            <span>
                                Quản lý nhóm
                            </span>
                            <span>
                                <span>
                                    <span class="material-symbols-rounded ms-2 sort-icon {{ $sortColumn == 'group_manager_id' ? '' : 'text-muted' }}">
                                        sort
                                    </span>
                                </span>
                            </span>
                    ...
                    @if(in_array('default_payment_account_id', $columns) || in_array('default_payment_account_id', $columns))
                    <th list-action="sort" sort-by="default_payment_account_id" sort-direction="{{ $sortColumn == 'default_payment_account_id' && $sortDirection ? $sortDirection : 'desc' }}" class="min-w-125px text-left" data-column="default_payment_account_id">
                        <span class="d-flex align-items-center">
                            <span>
                            ...
                            <span>
                                <span>
                                    <span class="material-symbols-rounded ms-2 sort-icon {{ $sortColumn == 'default_payment_account_id' ? '' : 'text-muted' }}">
                                        sort
                                    </span>
                                </span>
                            </span>
                ...
                <tr list-control="item">
                    <td class="text-left ps-1">
                        <div class="form-check form-check-sm form-check-custom">
                            <input data-item-id="{{ $staffGroup->id }}" list-action="check-item" class="form-check-input" type="checkbox" value="1" />
                        </div>
                    </td>
                    ...
                    @if(in_array('group_manager_id', $columns) || in_array('all', $columns))
                    <td data-column="group_manager_id" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                        {{ \App\Models\Account::find($staffGroup->group_manager_id)->name }}
                    </td>
                    @endif

                    @if(in_array('type', $columns) || in_array('all', $columns))
                    <td data-column="type" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                        {{ trans('messages.staff_groups.type.' . $staffGroup->type) }}
                    </td>
                    @endif

                    @if(in_array('default_payment_account_id', $columns) || in_array('all', $columns))
                    <td data-column="default_payment_account_id" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                        {{ \App\Models\PaymentAccount::find($staffGroup->default_payment_account_id)->account_name }} -
                        {{ \App\Models\PaymentAccount::find($staffGroup->default_payment_account_id)->account_number }} -
                        {{ \App\Models\PaymentAccount::find($staffGroup->default_payment_account_id)->bank }}
                    </td>
                    @endif
                    ...
                    <td data-column="created_at" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                        ...
                        {{ $staffGroup->created_at ? date('d/m/Y', strtotime($staffGroup->created_at)) : '--' }}
                    </td>
                    @endif
                    ...
                    <td data-column="updated_at" class="text-left mb-1 text-nowrap" data-filter="mastercard">
                        {{ $staffGroup->updated_at ? date('d/m/Y', strtotime($staffGroup->updated_at)) : '--' }}
                    </td>
                    @endif
                    ...
                    <td data-column="action">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                            ...
                            <div class="menu-item px-3">
                                <a row-action="delete"
                                    href="{{ action(
                                        [App\Http\Controllers\System\StaffGroupController::class, 'destroy'],
                                        [
                                            'id' => $staffGroup->id,
                                        ],
                                    ) }}"
                            ...
                            <div class="menu-item px-3">
                                <a row-action="edit"
                                    href="{{ action(
                                        [App\Http\Controllers\System\StaffGroupController::class, 'edit'],
                                        [
                                            'id' => $staffGroup->id,
                                        ],
                                    ) }}"
    ...
    <div class="py-15">
        ...
        <p class="text-center">
            <a href="javascript:;" class="btn btn-outline btn-outline-default">
    ...
    @endif
        <script>
            ...
            var HorizonScrollFix = function() {
                let staffGroupsBox = $('#staff-groups-table-box');
                ...
                let applyScroll = () => {
                    staffGroupsBox.scrollLeft(window.ordersListScrollFromLeft);
                }
                ...
                return {
                    init: function() {
                        ...
                        staffGroupsBox.on('scroll', () => {
                            setScroll(staffGroupsBox.scrollLeft());
                        });
                    }
                }
            }();
