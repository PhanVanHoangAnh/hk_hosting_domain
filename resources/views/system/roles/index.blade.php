...
@section('content')
    ...
    <div class="toolbar d-flex flex-stack flex-wrap mb-5 mb-lg-7" id="kt_toolbar">
        ...
        <div class="page-title d-flex flex-column py-1">
            ...
            <h1 class="d-flex align-items-center my-1">
                <span class="text-dark fw-bold fs-1">Danh Sách Vai Trò</span>
            </h1>
            <!--end::Title-->
            <!--begin::Breadcrumb-->
        ...
        <div class="d-flex align-items-center py-1">
            ...
            <!--begin::Button-->
            <a href="#" class="btn btn-flex btn-sm btn-primary fw-bold border-0 fs-6 h-40px" id="buttonCreateRole">
    ...
    <!--end::Toolbar-->
    <!--begin::Post-->
    <div id="RolesIndexContainer" class="post" id="kt_post">
        <!--begin::Card-->
        <div class="card">
            <!--begin::Card header-->
            ...
            <div class="card-header border-0 p-4 d-none list-filter-box pt-0 pb-0" list-action="filter-action-box">
                <!--begin::Card toolbar-->
                <div class="ccard-toolbar mb-0 w-100 d-flex" list-action="tool-action-box">
                    <div data-control="filters" class="row w-100">

                        <div class="col-md-4 mb-5">
                            <label class="form-label">Phân hệ</label>
                            <select data-control="module-select" name="module" class="form-select filter-select @if ($errors->has('module')) is-invalid @endif" data-control="select2"
                                data-close-on-select="false" data-placeholder="Chọn phân hệ" data-allow-clear="true"
                                >
                                    <option value="">Chọn phân hệ</option>
                                    @foreach (App\Library\Module::getAll() as $module)
                                        <option value="{{ $module }}">{{ trans('messages.module.' . $module) }}</option>
                                    @endforeach
                            </select>
                        </div>
    ...
    <script>
        ...
        var FilterManager = class {
            constructor() {
                var filterBox = $(RolesList.getList().container).find('[data-control="filters"]')
                //
                filterBox.find(':input').on('change', function() {
        ...
        var RolesIndex = function() {
            return {
                init: function() {
                    ...
                    // Create Campaign
                    CreateRolePopup.init();
                    // Update Campaign
                    UpdateRolePopup.init();
            ...
            return {
                init: function() {
                    ...
                    popupCreateRole = new Popup({
                        url: "{{ action('App\Http\Controllers\System\RoleController@create') }}",
                    });

                    // create campaign button
                    buttonCreateRole = document.getElementById('buttonCreateRole');
        ...
        var RolesList = function() {
            ...
            return {
                init: function() {
                    list = new DataList({
                        url: "{{ action('\App\Http\Controllers\System\RoleController@list') }}",
                        container: document.querySelector('#RolesIndexContainer'),
                        listContent: document.querySelector('#RolesIndexListContent'),
                    });
                    list.load();
                },
        ...
        var DataList = class {
                    ...
                    this.getUpdateButtons().forEach(button => {
                        button.addEventListener('click', (e) => {
                            ...
                            var url = button.getAttribute('href');
                             ...
                             // alert(url);
                             UpdateRolePopup.getPopup().url = url;
                            UpdateRolePopup.getPopup().load();
                        });
                    });
                }
            }
            ...
            deleteItem(url) {
                // success alert
                ASTool.confirm({
                    message: 'Bạn có chắc chắn muốn xóa vai trò này chứ?',
                    ok: function() {
                        // effect
            ...
            load() {
                ...
                this.listXhr = $.ajax({
                    ...
                    data: {
                        ...
                        module: $(this.container).find('[data-control="module-select"]').val()
                    }
                }).done((content) => {
