        ...
        <div class="card">
            ...
            <div class="card-header border-0 px-4">
                ...
                <div list-action="top-action-box" class="menu d-flex justify-content-end align-items-center d-none"
                    ...
                    <div data-kt-menu-trigger="hover" data-kt-menu-placement="bottom-start" class="menu-item d-none">
                        <!--begin::Menu link-->
    ...
    <script>
        ...
        var FilterData = (function() {
            ...
            return {
                ...
                init: () => {
                    ...
                    // module selecy
                    $('[data-control="module-select"]').on('change', function() {
                        //
                        UsersList.getList().setModule($(this).val());;
                        UsersList.getList().load();
                    });
                }
            };
            ...
            return {
                init: function() {
                    popupDeleteUsers = new Popup({
                        url: "{{ action('\App\Http\Controllers\System\UserController@deleteUsers') }}",
                    });
            ...
            return {
                init: function() {
                    ...
                    popupCreateUser = new Popup({
                        url: "{{ action('\App\Http\Controllers\System\UserController@create') }}",
                    });
        ...
        var UsersList = function() {
            ...
            return {
                init: function() {
                    list = new DataList({
                        url: "{{ action('\App\Http\Controllers\System\UserController@list') }}",
                ...
                if (this.getListItemsCount()) {
                    ...
                    this.getDeleteButtons().forEach(button => {
                        button.addEventListener('click', (e) => {
                            ...
                            $.ajax({
                                url: "{{ action('\App\Http\Controllers\System\UserController@checkAbroadApplications', '') }}/" + userId,
                                method: 'GET',
                            }).done((response) => {
                                if (response.hasModule) { 
                                    ASTool.confirm({
                                        message: `Bạn đang có ${response.count} hồ sơ du học. Hồ sơ sẽ được chuyển về lại cho Trưởng nhóm, nếu không có Trưởng nhóm thì sẽ chyển về cho Quản lý bộ phận. Bạn có chắc muốn xóa tài khoản này chứ ?`,
                                        ok: () => this.deleteItem(url)
                                    });
                                } else { 
                                    ASTool.confirm({
                                        message: 'Bạn có chắc muốn ngưng hoạt động tài khoản này chứ?',
                                        ok: () => this.deleteItem(url)
                                    });
                                }
                            });
