                ...
                <thead>
                    <tr class="text-start bg-info text-light fw-bold fs-7 text-uppercase gs-0 text-nowrap">
                        ...
                        @if (in_array('module', $columns) || in_array('all', $columns))
                            <th class="min-w-125px" data-column="module">
                                <span>Phân hệ</span>
                            </th>
                        @endif
                        @if (in_array('role', $columns) || in_array('all', $columns))
                            <th class="min-w-125px" data-column="role">
                                <span class="d-flex align-items-center">
                        ...
                        @if (in_array('mentor', $columns) || in_array('all', $columns))
                            <th class="min-w-125px" data-column="mentor">
                                <span class="d-flex align-items-center">
                                    <span>Mentor</span>
                                </span>
                            </th>
                ...
                <tbody class="text-gray-600">
                    @foreach ($users as $user)
                        <tr list-control="item">
                            ...
                            @if (in_array('name', $columns) || in_array('all', $columns))
                                <td class="text-left text-nowrap" data-control="freeze-column" data-filter="mastercard"
                                    data-column="name">
                                    <div class="symbol symbol-40px me-3 d-none">
                                        ...
                                        @if (Storage::disk('public')->exists("users/{$user->id}/profile_picture.jpg"))
                                            <img width="30"
                                                src="{{ asset("/storage/users/{$user->id}/profile_picture.jpg") }}"
                                                alt="User Image">
                                        @else
                                            <img width="30" src="/core/assets/media/avatars/blank.png"
                                                alt="Default Image">
                                        @endif
                                        {{-- <img src="/storage/users/13/profile_picture.jpg"> --}}
                                    </div>
                                    <a href="javascript:;" class="text-gray-800 text-hover-primary text-nowrap">
                                        {{ $user->name }}
                                    </a>
                                </td>
                            @endif
                                ...
                                <td data-filter="mastercard" data-column="status">
                                    @php
                                        $bgs = [
                                            ...
                                            App\Models\User::STATUS_OUT_OF_JOB => 'warning',
                                        ];
                                    @endphp
                                
                                    <span title=""
                                        class="badge bg-{{ $bgs[$user->status] ?? 'info' }}"
                                        data-bs-toggle="tooltip" data-bs-trigger="hover"
                                        data-bs-dismiss="click" data-bs-placement="right"
                                    >
                                        {{ trans('messages.users.status.' . $user->status) }}
                                    </span>
                            ...
                            @if (in_array('module', $columns) || in_array('all', $columns))
                                <td class="text-left" data-filter="mastercard" data-column="module">
                                    @php
                                        $modules = $user->roles->map(function($role) {
                                            return $role->module;
                                        })->unique();
                                    @endphp
                                    @foreach ($modules as $module)
                                        <span
                                            class="badge badge-secondary">{{ trans('messages.module.' . $module) }}</span>
                                    @endforeach
                                </td>
                            @endif
                                ...
                                <td class="text-left" data-filter="mastercard" data-column="role">
                                    @foreach ($user->roles as $role)
                                        <span class="badge badge-secondary mb-1">
                                            <span class="d-flex align-items-center">
                                                <span>{{ trans('messages.module.' . $role->module) }}</span> <span
                            ...
                            <td class="">
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                    ...
                                    <div class="menu-item px-3">
                                        <a row-action="update"
                                            href="{{ action(
                                                [App\Http\Controllers\System\UserController::class, 'edit'],
                                                [
                                                    'id' => $user->id,
                                                ],
                                            ) }}"
                                    ...
                                    @if ($user->isActive())
                                        <div class="menu-item px-3 ">
                                            <a row-action="delete" data-id="{{ $user->id }}"
                                                href="{{ action([App\Http\Controllers\System\UserController::class, 'destroy'], ['id' => $user->id]) }}"
                                                class="menu-link px-3">Ngưng tài khoản</a>
                                        </div>
                                    @endif
