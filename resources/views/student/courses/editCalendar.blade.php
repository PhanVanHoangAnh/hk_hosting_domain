...
<div>
    ...
    <div class="card-header border-0" id="addCourseContainer">
        <form id="editCalendarForm" action="{{ action([App\Http\Controllers\Student\CourseController::class, 'updateCalendar'], ['id' => $course->id]) }}" method="put">
            @csrf
            @include('student.courses._form_duiedit_calendar')
        </form>
    </div>
