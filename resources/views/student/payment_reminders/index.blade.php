...
@section('content')
    ...
    <div id="PaymentRemindersIndexContainer" class="post" id="kt_post">
        @foreach($reminds as $remind)
            <div class="alert alert-info d-flex align-items-center p-5 mb-5">
                <i class="ki-duotone ki-shield-tick fs-2hx text-primary me-4"><span class="path1"></span><span class="path2"></span></i>                    <div class="d-flex flex-column">
                    ...
                    <div>Bạn đang có 1 hợp đồng <span class="fw-bold">{{ $remind->order->code }}</span> tới hạn thanh toán. <a 
                    href="{{ action(
                        [App\Http\Controllers\Student\OnePayController::class, 'onepayPayment'],
                        ['amount' => $remind->getDebtAmountInProgress()]
                    ) }}"  
                    class="fw-semibold d-inline-block">Nhấn vào đây</a> để thanh toán hợp đồng.</div>
                </div>
            </div>
        @endforeach
        <!--begin::Card-->
                            ...
                            <div class="row" list-action="created_at-select">
                                <div class="form-outline">
                                    <select id="order-filter-contact-select" data-control="select2-ajax"
                                        data-url="{{ action('App\Http\Controllers\Student\ContactController@select2') }}"
        ...
        <input type="hidden" id="paymentSuccess" value="{{ session('payment_success') }}">
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            var paymentSuccess = document.getElementById('paymentSuccess').value;
        ...
        var PaymentRemindersList = function() {
            ...
            return {
                init: function() {
                    list = new DataList({
                        url: "{{ action('\App\Http\Controllers\Student\PaymentReminderController@list') }}",
        ...
        var FilterData = (function() {
            ...
            return {
                init: () => {
                    ...
                    $('[list-action="due_date-select"]').on('change', function() {
                        // Lấy giá trị từ cả hai trường input
