...
@php
    $formId = 'editApplicationSubmissionPopup_' . uniqid();
@endphp

@section('content')
<div>
    <form id="{{ $formId }}" method="POST" tabindex="-1" enctype="multipart/form-data"
        action="{{ action([App\Http\Controllers\Student\AbroadController::class, 'updateApplicationSubmission'], ['id' => $applicationSubmission->id]) }}">
        @csrf   
        <div class="pe-7 py-5   px-lg-17">
                ...
                <div class="col-md-12 fv-row ">
                    ...
                    <option value="">Chọn trường </option>
                        @foreach(\App\Models\School::all()  as $school)
                            <option value="{{ $school->id }}" {{ isset($applicationSubmission) && $applicationSubmission->school_id == $school->id ? 'selected' : '' }}> {{ $school->name }} </option>
                        @endforeach
                    </select>
                    <!--end::Input-->
            ...
            <div class="row py-3">
                <div class="col-md-12 fv-row">
                    ...
                    class="form-control" placeholder="" value="{{ $applicationSubmission->deadline }}" required />
...
<script>
    $(() => {
        EditApplicationSubmission.init();
    })

    var EditApplicationSubmission = function() {
        let form;
        let submitBtn;
