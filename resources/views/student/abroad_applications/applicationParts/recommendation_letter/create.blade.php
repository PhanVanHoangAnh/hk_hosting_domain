...
@section('content')
<div>
    <form id="{{ $createRecommendationLetterPopup }}" method="POST" action="{{ action([App\Http\Controllers\Student\AbroadController::class, 'updateDepositData'], ['id' => $abroadApplication->id]) }}" tabindex="-1" enctype="multipart/form-data" 
        action="{{ action([App\Http\Controllers\Student\AbroadController::class, 'storeRecommendationLetter'], ['id' => $abroadApplication->id]) }}">
