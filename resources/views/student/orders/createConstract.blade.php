    ...
    <div id="create-constract-content">
        ...
        <div class="card-header border-0 mb-10" id="create-constract-container">
            <form id="create-constract-form">
                        ...
                        <script>
                            $(function() {
                                new ContactInfoBox({
                                    ...
                                    url: '{{ action('\App\Http\Controllers\Student\ContactController@infoBox', 'CONTACT_ID') }}',
                                });
                            });
                        </script>
                    </div>
                        ...
                        <div class="col-6">
                            <div class="form-outline">
                                ...
                                @include('helpers.contactSelector', [
                                    ...
                                    'createUrl' => action('\App\Http\Controllers\Student\ContactController@create'),
                                    'editUrl' => action(
                                        '\App\Http\Controllers\Student\ContactController@edit',
                                        'CONTACT_ID'),
                                    'editOnly' => true,
                                ])
                ...
                {{-- SALES MANAGER --}}
                @include('student.orders._salesManager')
                ...
                @if ($order->type != \App\Models\Order::TYPE_REQUEST_DEMO)
                    <div class="form-outline mt-5 d-flex justify-content-start" section-control>
                        ...
                        <button id="addExtraOrder" data-href="{{ action([App\Http\Controllers\Student\OrderController::class, 'showCreateExtraPopup'], ['orderId' => $order->id]) }}"
                                                            ...
                                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                                                ...
                                                                <div class="menu-item px-3">
                                                                    <a data-order-item-id="{{ $orderItem->id }}"
                                                                        ...
                                                                        data-link="{{ action(
                                                                            [App\Http\Controllers\Student\OrderItemController::class, 'copy'],
                                                                            [
                                                                                'id' => $orderItem->id,
                                                                            ],
                                                                        ) }}"
                                                                ...
                                                                <div class="menu-item px-3">
                                                                    <a data-order-item-id="{{ $orderItem->id }}"
                                                                        href="{{ action(
                                                                            [App\Http\Controllers\Student\OrderItemController::class, 'delete'],
                                                                            [
                                                                                'id' => $orderItem->id,
                                                                            ],
                                                                        ) }}"
                ...
                @if ($order->isEdu())
                    @include('student.orders._eduPriceManager')
                @endif

                {{-- ABROAD ORDER PRICE MANAGER--}}
                @if ($order->isAbroad())
                    @include('student.orders._abroadPriceManager')
                @endif

                {{-- ABROAD ORDER PRICE MANAGER--}}
                @if ($order->isExtra())
                    @include('student.orders._extraPriceManager')
                @endif
        ...
        <script>
            ...
            var Constract = class {
                ...
                showCreateTraiOrderPopup() {
                    AddTrainOrderPopup.updateUrl(
                        "{!! action('App\Http\Controllers\Student\OrderController@createTrainOrder', ['contactId' => $order->contact_id, 'orderId' => $order->id,]) !!}"
                    );
                };

                showCreateExtraItemPopup(url) {
                    AddExtraItemPopup.updateUrl(
                        "{!! action([App\Http\Controllers\Student\OrderController::class, 'showCreateExtraPopup'], ['orderId' => $order->id]) !!}"
                    );
                };

                showCreateDemoOrderPopup() {
                    AddDemoOrderPopup.updateUrl(
                        "{!! action([App\Http\Controllers\Student\OrderController::class, 'createDemoOrder'], ['orderId' => $order->id]) !!}"
                    );
                };

                showCreateAbroadOrderPopup() {
                    AddAbroadOrderPopup.updateUrl(
                        "{!! action([App\Http\Controllers\Student\OrderController::class, 'createAbroadOrder'], ['orderId' => $order->id]) !!}"
                    );
                };
                ...
                saveConstractData() {
                    ...
                    $.ajax({
                        url: "{{ action('App\Http\Controllers\Student\OrderController@saveConstractData') }}",
                        method: "POST",
                        data: formData
                    }).done(response => {
                ...
                confirmAndRequestApproveConstractData() {
                    ...
                    $.ajax({
                        url: "{{ action('App\Http\Controllers\Student\OrderController@doneCreateConstractData') }}",
                        method: "POST",
                        data: formData
                    }).done(response => {
                        this.removeSubmitEffect();
                        ...
                        ASTool.alert({
                            ...
                            ok: () => {
                                ...
                                window.location.href =
                                    `{{ action('App\Http\Controllers\Student\OrderController@index', ['type' => '']) }}${screenType}`;
                            }
                        });
                    }).fail(response => {
                        this.removeSubmitEffect();
                    ...
                    if (this.getUpdateTrainOrderBtns()) {
                        this.getUpdateTrainOrderBtns().forEach(button => {
                            $(button).on('click', function(e) {
                                ...
                                const url = " {!! action('App\Http\Controllers\Student\OrderController@createTrainOrder', ['orderItemId' => 'PLACEHOLDER', 'contactId' => $order->contact_id, 'orderId' => $order->id]) !!} ";
                    ...
                    if (this.getUpdateDemoOrderBtns()) {
                        this.getUpdateDemoOrderBtns().forEach(button => {
                            button.addEventListener('click', function(e) {
                                ...
                                const orderItemId = this.getAttribute('data-order-item-id');
                                const url = " {!! action('App\Http\Controllers\Student\OrderController@createDemoOrder', ['orderItemId' => 'PLACEHOLDER', 'contactId' => $order->contact_id, 'orderId' => $order->id]) !!} ";
                                const updatedUrl = url.replace('PLACEHOLDER', orderItemId);
                    ...
                    if (this.getUpdateAbroadOrderBtns()) {
                        this.getUpdateAbroadOrderBtns().forEach(button => {
                            ...
                            button.addEventListener('click', function(e) {
                                ...
                                const orderItemId = this.getAttribute('data-order-item-id');
                                const url = " {!! action('App\Http\Controllers\Student\OrderController@createAbroadOrder', ['orderItemId' => 'PLACEHOLDER', 'contactId' => $order->contact_id, 'orderId' => $order->id]) !!} ";
                                const updatedUrl = url.replace('PLACEHOLDER', orderItemId);
                    ...
                    if (this.getUpdateExtraOrderBtns()) {
                        this.getUpdateExtraOrderBtns().forEach(button => {
                            ...
                            button.addEventListener('click', function(e) {
                                ...
                                const orderItemId = this.getAttribute('data-order-item-id');
                                const url = " {!! action('App\Http\Controllers\Student\OrderController@showCreateExtraPopup', ['orderItemId' => 'PLACEHOLDER', 'contactId' => $order->contact_id, 'orderId' => $order->id]) !!} ";
                                const updatedUrl = url.replace('PLACEHOLDER', orderItemId);
                ...
                setPopup(type) {
                    switch (type) {
                        case EDU_TYPE:
                            ...
                            this.popupLink = "{!! action('App\Http\Controllers\Student\OrderController@createTrainOrder', ['orderItemId' => 'PLACEHOLDER', 'currOrderItemId' => 'CURR_ID']) !!}";
                            break;
                        case ABROAD_TYPE:
                            this.editPopup = AddAbroadOrderPopup;
                            this.popupLink = "{!! action('App\Http\Controllers\Student\OrderController@createAbroadOrder', ['orderItemId' => 'PLACEHOLDER', 'currOrderItemId' => 'CURR_ID']) !!}";
                            break;
                        case DEMO_TYPE:
                            this.editPopup = AddDemoOrderPopup;
                            this.popupLink = "{!! action('App\Http\Controllers\Student\OrderController@createDemoOrder', ['orderItemId' => 'PLACEHOLDER', 'currOrderItemId' => 'CURR_ID']) !!}";
                            break;
                        case EXTRA_TYPE:
                            this.editPopup = AddExtraItemPopup;
                            this.popupLink = "{!! action('App\Http\Controllers\Student\OrderController@showCreateExtraPopup', ['orderItemId' => 'PLACEHOLDER', 'currOrderItemId' => 'CURR_ID']) !!}";
                            break;
                        default:
                            this.editPopup = AddTrainOrderPopup;
                            this.popupLink = "{!! action('App\Http\Controllers\Student\OrderController@createTrainOrder', ['orderItemId' => 'PLACEHOLDER', 'currOrderItemId' => 'CURR_ID']) !!}";
            ...
            var OrderForm = class {
                #url = "{{ action('App\Http\Controllers\Student\OrderController@saveOrderItemData') }}";
                #orderId = "{{ $orderId }}";
            ...
            var PriceManager = class {
                ...
                loadOrderPrice() {
                    ...
                    $.ajax({
                        url: '{{ action('App\Http\Controllers\Student\OrderController@getTotalPriceOfItems') }}',
                        data: {
                            _token: "{{ csrf_token() }}",
                            id: _this.getOrderId()
                        },
