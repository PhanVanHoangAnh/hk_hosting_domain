...
@section('title')
    ...
    @if (isset($copyFromOrder))
        (Sao chép thông tin từ hợp đồng #{{ $copyFromOrder->code }})
    @endif
@endsection
...
@section('content')
<form id="{{ $formId }}" method="POST" action="{{ action('\App\Http\Controllers\Student\OrderController@createConstract') }}">
    @csrf

    <div class="mt-10">
        <div class="row w-100 px-15 mx-auto d-flex justify-content-around mt-10 {{ isset($copyFromOrder) ? 'd-none' : '' }}">
            <div class="col-12">
                <label for="" class="form-label fw-semibold text-info">Loại hợp đồng</label>
            </div>
        </div>

        <div class="pb-10 {{ isset($copyFromOrder) ? 'd-none' : '' }}">
        ...
        <div class="row w-100 px-15 mx-auto d-flex justify-content-around mb-10">
            <div class="col-12">
                @include('student.orders._contact_form', [
                    'formId' => $formId,
        ...
        return {
            init: () => {
                ...
                form.addEventListener('submit', e => {
                    ...
                    addSubmitEffect();
                        $.ajax({
                            url: "{{ action('\App\Http\Controllers\Student\OrderController@createConstract') }}",
                            method: 'POST',
                            data: data
                        }).done(response => {
