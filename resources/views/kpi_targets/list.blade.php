            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
                <thead>
                    <tr class="text-start bg-info text-light fw-bold fs-7 text-uppercase gs-0 text-nowrap">
                        ...
                        @if (in_array('account_code', $columns) || in_array('all', $columns))
                            <th list-action="sort" sort-by="accounts.code"
                                sort-direction="{{ $sortColumn == 'accounts.code' && $sortDirection ? $sortDirection : 'desc' }}"
                                class="min-w-125px text-left" data-column="account_code"
                                width="1%"
                            >
                                <span class="d-flex align-items-center">
                                    ...
                                    <span>
                                        <span>
                                            <span
                                                class="material-symbols-rounded ms-2 sort-icon {{ $sortColumn == 'accounts.code' ? '' : 'text-muted' }}">
                                                sort
                                            </span>
                                        </span>
                                    </span>
                        ...
                        @if (in_array('account_id', $columns) || in_array('all', $columns))
                            <th list-action="sort" sort-by="accounts.name"
                                sort-direction="{{ $sortColumn == 'accounts.name' && $sortDirection ? $sortDirection : 'desc' }}"
                                class="min-w-125px text-left" data-column="account_id">
                                <span class="d-flex align-items-center">
                                    <span>
                                        Nhân viên
                                    </span>
                                    <span>
                                        <span>
                                            <span
                                                class="material-symbols-rounded ms-2 sort-icon {{ $sortColumn == 'accounts.name' ? '' : 'text-muted' }}">
                                                sort
                                            </span>
                                        </span>
                                    </span>
                        ...
                        @if (in_array('amount', $columns) || in_array('all', $columns))
                            <th list-action="sort" sort-by="amount"
                                ...
                                class="min-w-125px text-left" data-column="amount">
                                <span class="d-flex align-items-center">
                                    <span>
                                        Chỉ tiêu
                                    </span>
                        ...
                        <th width="1%" class="fs-8 min-w-100px">Thao tác</th>
                    </tr>
                </thead>
                        ...
                        <tr list-control="item">
                            <td class="ps-1">
                                <div class="form-check form-check-sm form-check-custom">
                                    <input list-action="check-item" name="checkbox-delete" class="form-check-input"
                                        type="checkbox" value="{{ $kpiTarget->id }}" />
                                </div>
                            </td>
                            ...
                            @if (in_array('created_at', $columns) || in_array('all', $columns))
                                <td data-column="created_at" class="text-left" data-filter="mastercard">
                                    {{ $kpiTarget->created_at->format('d/m/Y') }}
                                </td>
                            @endif
                            ...
                            <td data-column="action">
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                    ...
                                    <div class="menu-item px-3">
                                        <a row-action="update"
                                            href="{{ action(
                                                [App\Http\Controllers\KpiTargetController::class, 'edit'],
                                                [
                                                    'id' => $kpiTarget->id,
                                                ],
                                            ) }}"
                                    ...
                                    <div class="menu-item px-3">
                                        <a row-action="delete"
                                            href="{{ action([App\Http\Controllers\KpiTargetController::class, 'delete'], [
                                                'id' => $kpiTarget->id
                                            ]) }}"
