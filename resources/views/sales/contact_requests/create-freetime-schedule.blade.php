...
@endphp
@section('content')
<form id="{{ $formId}}" tabindex="-1" enctype="multipart/form-data" method="post" action="{{ action( [App\Http\Controllers\Sales\ContactRequestController::class, 'saveFreetimeSchedule'],
    [
        'id' => $contact->id,
    ],
) }}">
        ...
        <div class="card p-5">
            ...
            <script>
                $(function () {
                    ...
                    if (container) {
                        var events = {!! json_encode($contact->busy_schedule ?: []) !!};
