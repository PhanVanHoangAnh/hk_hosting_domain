...
@section('content')
    ...
    <form id="{{ $formId }}" action="{{ action([App\Http\Controllers\Sales\ContactRequestController::class, 'store']) }}" method="post"  enctype="multipart/form-data">
        @csrf
            ...
            $.ajax({
                ...
                ASTool.alert({
                    ...
                    ok: function() {
                        ...
                        @if (isset($orderType))
                            if (typeof pickContactPopup != 'undefined') pickContactPopup.updateUrl("{{ action([App\Http\Controllers\Sales\OrderController::class, $orderType == \App\Models\Order::TYPE_REQUEST_DEMO ? 'pickContactForRequestDemo' : 'pickContact'], ['contact_id' => isset($contactRequest->contact_id) ? $contactRequest->contact_id : '']) }}");
                        @endif
                        // Trick
                        setTimeout(() => {
