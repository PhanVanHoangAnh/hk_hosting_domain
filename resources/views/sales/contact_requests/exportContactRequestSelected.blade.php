...
@section('content')
    <form id="formExport" action="{{ action('App\Http\Controllers\Sales\ContactRequestController@exportContactRequestSelectedRun') }}"
    ...
    <script>
        ...
        document.getElementById('exportSelectContactRequestButton').addEventListener('click', function(event) {
            ...
            $.ajax({
                ...
                window.location =
                    "{{ action('App\Http\Controllers\Sales\ContactRequestController@exportContactRequestSelectedDownload') }}?file=" + response.file;
