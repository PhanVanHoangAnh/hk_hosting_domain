...
<div class="row">
    ...
    data-upload-url="{{ action([App\Http\Controllers\Sales\AbroadController::class, 'storeDepositFile'], ['id' => $abroadApplication->id]) }}"
    data-delete-url="{{ action([App\Http\Controllers\Sales\AbroadController::class, 'deleteDepositFile'], ['id' => $abroadApplication->id]) }}"
    >
        ...
        <form form-data="deposit-form" method="POST" action="{{ action([App\Http\Controllers\Sales\AbroadController::class, 'updateDepositData'], ['id' => $abroadApplication->id]) }}" class="row d-flex justify-content-around">
            @csrf
            <div class="col-md-4">
                {{ isset($abroadApplication->deposit_cost) ? $abroadApplication->deposit_cost : '' }}
            </div>
            <div class="col-md-4">
                <span class="badge badge-secondary">{{ trans('messages.deposit_fee.' . $abroadApplication->deposit_status) }}</span>
            </div>
            <div class="col-md-4">
