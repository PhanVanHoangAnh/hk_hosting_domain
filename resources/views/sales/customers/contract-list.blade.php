<div class="card-body pt-0 mt-5">
                    ...
                    <tr class="text-start bg-info text-light fw-bold fs-7 text-uppercase gs-0 text-nowrap">
                        <th class="w-10px pe-2 ps-1">
                            ...
                            <th list-action="sort" sort-by="price"
                                ...
                                class="min-w-100px fs-8" data-column="price">
                                <span class="d-flex align-items-center">
                                    <span>
                                        Giá hợp đồng
                                    </span>
                            ...
                            <th list-action="sort" sort-by="sale"
                                ...
                                class="min-w-100px fs-8" data-column="sale">
                                <span class="d-flex align-items-center">
                                    <span>
                                       Sale
                                    </span>
                            ...
                            <th list-action="sort" sort-by="sale_sup"
                                ...
                                class="min-w-100px fs-8" data-column="sale_sup">
                                <span class="d-flex align-items-center">
                                    <span>
                                       Sale Sup
                                    </span>
                    ...
                    @foreach ($orders as $order)
                        <tr list-control="item">
                            <td class="ps-1">
                                ...
                                <td data-column="code">{{ $order->code }}</td>
                                <td data-column="code">{{ $order->type }}</td>
                                <td data-column="code">{{ $order->fullname }}</td>
                                <td data-column="code">  {{ App\Helpers\Functions::formatNumber($order->sumAmountPaid()) }}₫</td>
                                <td data-column="code">     {{App\Helpers\Functions::formatNumber(max($order->getTotal() - $order->sumAmountPaid(), 0), 0, '.', ',') }}₫</td>
                                <td data-column="code">  {{ App\Helpers\Functions::formatNumber($order->getTotal()) }}₫</td>
                                <td data-column="code">
                                    @php
                                        $bgs = [
                                    ...
                                    @endphp
                                    @if ($order->isRejected())
                                        <span title="{{ $order->rejected_reason }}"
                                ...
                                <td data-column="code">{{ $order->updated_at->format('d/m/Y') }}</td>
                                <td data-column="code"> {{ $order->created_at->format('d/m/Y') }}</td>
                                <td data-column="code"> {{ App\Models\Account::find($order->sale) ? App\Models\Account::find($order->sale)->name : '' }}</td>
                                <td data-column="code"> {{ App\Models\Account::find($order->sale_sup) ? App\Models\Account::find($order->sale)->name : '' }}</td>
                                <td class="text-left">
                                <a href="#"
                                    ...
                                    @if (Auth::user()->can('requestApproval', $order))
                                        <div class="menu-item px-3">
                                            <a row-action="request-approval"
                                                ...
                                                class="menu-link px-3">Xác nhận & Yêu cầu duyệtss</a>
                                        </div>
                                    @endif
    ...
    @else
        <div class="py-15">
            ...
            <p class="text-center">
                <a class="btn btn-outline btn-outline-default" id="addContractCustomerDetail" >
                    <span class="d-flex align-items-center">
    ...
    <script>
        $(() => {
            ...
            if (document.getElementById('addContractCustomerDetail') !== null) {
                CreateConstract.init()
            }
        });
        var CreateConstract = function() {
            let createOrderBtns = document.getElementById('addContractCustomerDetail');

            return {
                init: () => {
                    createOrderBtns.addEventListener('click', e => {
                        e.preventDefault();
        ...
        var RequestApproval = function() {
            ...
            return {
                init: function() {
                    ...
                    links = list.listContent.querySelectorAll('[row-action="request-approval"]');
