...
@section('title')
    Lịch sử ghi chú của học viên {{ $contact->contact->name }}
@endsection
                ...
                init: () => {
                    ...
                    buttonsUpdateNoteLog.forEach(function(button) {
                        button.addEventListener('click', function(e) {
                            ...
                            updatePopups = new Popup();
                            updatePopups.url = editUrl;
                            updatePopups.load();
                        });
                    });
                },
                    ...
                    buttonsCreateNoteLog.forEach(function(button) {
                        button.addEventListener('click', function(e) {
                            ...
                            CreateNotePopup.updateUrl(
                                "{{ action('\App\Http\Controllers\Sales\NoteLogController@createNoteLogCustomer', ['id' => $contact->id]) }}"
                            );
                        });
                    });
                }
