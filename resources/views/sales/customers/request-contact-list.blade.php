                ...
                <thead>
                    <tr class="text-start bg-info text-light fw-bold fs-7 text-uppercase gs-0 text-nowrap">
                        <th class="w-10px pe-2 ps-1">
                            ...
                            <th list-action="sort" sort-by="demand"
                    ...
                    @foreach ($contactRequests as $contactRequest)
                        <tr list-control="item">
                            <td class="ps-1">
                                ...
                                <td data-column="code">{{ $contactRequest->demand }}</td>
                                <td data-column="code">{{ $contactRequest->lead_status }}</td>
                                <td data-column="code">{{ $contactRequest->account ? $contactRequest->account->name : '' }}</td>
                                <td data-column="code">{{ $contactRequest->source_type }}</td>
                                <td data-column="code">{{ $contactRequest->channel }}</td>
                                <td data-column="code">{{ $contactRequest->sub_channel }}</td>
                                <td data-column="code">{{ $contactRequest->lifecycle_stage }}</td>
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                    ...
                                    @if (!$contactRequest->hasOrders())
                                        ...
                                        <div class="menu-item px-3">
                                            <a list-action="add-order" href="{{ action(
...
$(() => {
    if (document.getElementById('addMarketingContactRequest') !== null) {
        CreateContactRequest.init()
    }
});

var CreateContactRequest = function() {
    let createRequestBtns = document.getElementById('addMarketingContactRequest');

    return {
        init: () => {
            createRequestBtns.addEventListener('click', e => {
                e.preventDefault();
                pickContactPopup.updateUrl("{{ action([App\Http\Controllers\Sales\ContactRequestController::class, 'create'], ['contact_id' => $contact->id]) }}");
            });
        }
    };
}();
