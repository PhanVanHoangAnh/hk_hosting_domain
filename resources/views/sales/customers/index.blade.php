...
@section('content')
    ...
    <div class="toolbar d-flex flex-stack flex-wrap mb-5 mb-lg-7" id="kt_toolbar">
        ...
        <div class="d-flex align-items-center py-1  d-none">
            <!--begin::Button-->
            <a href="#" class="btn btn-flex btn-sm btn-primary fw-bold border-0 fs-6 h-45px" id="buttonCreateContact"
    ...
    <!--end::Toolbar-->
    <!--begin::Post-->
    <div id="CustomersIndexContainer" class="position-relative" id="kt_post">
        <!--begin::Card-->
        <div class="card">
            <!--begin::Card header-->
                        ...
                        <div class="menu-sub menu-sub-dropdown p-3 w-200px">
                            ...
                            <div class="menu-item d-none">
                                <a href="#" class="menu-link px-3">
                                    ...
                                    <span data-kt-customer-table-select="selected_count" class="menu-title "
                            ...
                            <div class="menu-item px-3">
                                <a row-action="delete-all" href="{{ action([App\Http\Controllers\Sales\ContactController::class, 'deleteAll']) }}"
                        ...
                        <div class="col-md-3 mb-5 d-none">
                            ...
                            <div>
                                <select list-action="salesperson-select" class="form-select" data-control="select2"
                                    ...
                                    @php
                                        $uniqueSalespersion = App\Models\Contact::pluck('account_id')->unique();
                                    @endphp
                                    @foreach ($uniqueSalespersion as $sales_person)
                                        @php
                                            $account = App\Models\Account::find($sales_person);
                                        @endphp
        ...
        <div id="CustomersIndexListContent">
        </div>
    ...
    <script>
        ...
        var NodeLogsCustomerPopup = function() {
            var popupNodeLogsCustomer;
            return {
                init: function(updateUrl) {
                    // create campaign popup
                    popupNodeLogsCustomer = new Popup({
                        url: updateUrl,

                    });
                },
                load: function(url) {
                    popupNodeLogsCustomer.url = url;
                    popupNodeLogsCustomer.load(); // Hiển thị cửa sổ chỉnh sử
                },

                getPopup: function() {
                    return popupNodeLogsCustomer;
                }
            };
        }();
            ...
            return {
                init: function() {
                    popupEditTags = new Popup({
                        url: "{{ action('\App\Http\Controllers\Sales\CustomerController@addTagsBulk') }}",
                    });
            ...
            return {
                init: function() {
                    popupDeleteTags = new Popup({
                        url: "{{ action('\App\Http\Controllers\Sales\CustomerController@deleteTags') }}",
                    });
        ...
        var CreateContactPopup = function() {
            ...
            // show campaign modal
            var showCustomerModal = function() {
        ...
        var FilterData = (function() {
            ...
            return {
                ...
                init: () => {
                    ...
                    $('[list-action="lead-status-select"]').on('change', function() {
                        var selectedValues = getSelectedValuesFromMultiSelect($(this)[0]);
                    ...
                    $('[list-action="export-excel"]').on('click', function() {  
                        ...
                        $.ajax({
                            url: "{{ action('\App\Http\Controllers\Sales\CustomerController@export') }}",
                                type: "GET",
                                data: query,
                                xhrFields: {
                                    responseType: 'blob' 
        ...
        var ContactsList = function() {
            ...
            return {
                init: function() {
                    list = new DataList({
                        url: "{{ action('\App\Http\Controllers\Sales\CustomerController@list') }}",
                        container: document.querySelector('#CustomersIndexContainer'),
                        listContent: document.querySelector(
                            '#CustomersIndexListContent'),
        ...
        var DataList = class {
            ...
            initContentEvents() {
                ...
                if (this.getListItemsCount()) {
                    ...
                    this.getNoteLogsCustomerButtons().forEach(button => {
                        button.addEventListener('click', (e) => {
                            e.preventDefault();

                            var url = button.getAttribute('href');
                            // show popup
                            NodeLogsCustomerPopup.load(url);
                        });
                    });
