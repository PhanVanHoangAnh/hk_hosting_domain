...
@section('content')
    ...
    <script>
            ...
            return {
                init: () => {
                    ...
                    noteList = new DataList({
                        ...
                        url: "{{ action('App\Http\Controllers\Sales\ContactController@noteLogList', ['id' => $contact->id]) }}",
        ...
        var createNotlogHandle = function() {
            ...
            return {
                init: () => {
                    document.querySelector('#buttonCreateNoteLogMarketting').addEventListener('click', e => {
                        e.preventDefault();

                        CreateNotePopup.updateUrl(
                            "{{ action('\App\Http\Controllers\NoteLogController@addNoteLogMarketingContactRequest', ['id' => $contact->id]) }}"
                        );
                    });
                }
            };
                ...
                if (this.getNumberOfItems()) {
                    ...
                    this.getPageUrl().forEach(url => {
                        ...
                        url.addEventListener('click', e => {
                            ...
                            let pageNumber = Number(url.getAttribute('href').slice(-1));
                ...
                tool.confirm({
                    ...
                    ok: function() {
                        ...
                        $.ajax({
                            ...
                            data: {
                                _token: "{{ csrf_token() }}",
                                noteIds: NoteList.getList().getNoteCheckedIds()
                            }
                        }).done(response => {
                            tool.alert({
                                message: response.message,
