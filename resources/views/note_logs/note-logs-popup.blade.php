...
@section('content')
            ...
            <div class="card-body pt-0">
                @if ($notes->count() || $noteMarketting)
                    <div class="table-responsive">
                        ...
                        @if ($notes->count())
                            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_contacts_table">
                                <thead>
                                    <tr
                                        class="text-start text-gray-800 fw-bold fs-7 text-uppercase gs-0 bg-light text-nowrap">
                                        ...
                                        <th class="min-w-125px text-nowrap" width="80px">Thao tác</th>
                                    </tr>
                                </thead>
                                                ...
                                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                                    ...
                                                    @if (Auth::user()->can('update', $note))
                                                        <div class="menu-item px-3">
                                                            <a href="{{ action(
                                                                [App\Http\Controllers\NoteLogController::class, 'edit'],
                                                                [
                                                                    'id' => $note->id,
                                                                ],
                                                            ) }}"
                    ...
                    <div class="modal-footer flex-end">
                        <!--begin::Button-->
                        <button type="button" class="buttonCreateNewNoteLog btn btn-primary">
                            Thêm ghi chú
                        </button>
                        <!--end::Button-->
                    </div>
                @else
                    <div class="py-15">
                        ...
                        <p class="text-center">
                            <a href="" class="btn btn-outline btn-outline-default buttonCreateNewNoteLog">
    ...
    <script>
                    ...
                    ok: function() {
                        ...
                        $.ajax({
                            ...
                            ASTool.alert({
                                ...
                                ok: function() {
                                    if (typeof ContactRequestsList !==
                                        'undefined') {
                                        if (typeof ContactRequestsList !=
                                            'undefined') ContactRequestsList
                                            .getList().load();
                                    }
        ...
        var AddNoteLog = function() {
            return {
                init: function() {
                    document.querySelectorAll('.buttonCreateNewNoteLog').forEach(button => {
                        button.addEventListener('click', e => {
                            e.preventDefault();
                            CreateNotePopup.updateUrl(
                                "{{ action('\App\Http\Controllers\NoteLogController@addNoteLogMarketingContactRequest', ['id' => $contactRequest->id]) }}"
                            );
                        });
                    });
                }
