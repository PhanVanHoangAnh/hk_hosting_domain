            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_contacts_table">
                <thead>
                    <tr class="text-start bg-info text-light fw-bold fs-7 text-uppercase gs-0 text-nowrap">
                        ...
                        class="min-w-125px text-left fs-8" data-column="contact_id">
                        ...
                        <th class="min-w-125px text-left fs-8">
                            <span>
                                Khách hàng
                            </span>
                        </th>
                        ...
                        <th class="min-w-125px ">Loại dịch vụ</th>

                        <th
                            class="min-w-125px" data-column="subject_id">
                            Chi tiết DV
                        </th>
                         
                        <th list-action="sort" sort-by="note"
                            sort-direction="{{ $sortColumn == 'note' && $sortDirection ? $sortDirection : 'desc' }}"
                            class="min-w-125px text-left fs-8" data-column="note">
                            <span class="d-flex align-items-center">
                                <span>
                                    Ghi chú
                                </span>
                        ...
                        <th list-action="sort" sort-by="estimated_payment_date"
                            sort-direction="{{ $sortColumn == 'estimated_payment_date' && $sortDirection ? $sortDirection : 'desc' }}"
                            class="min-w-125px text-left fs-8" data-column="estimated_payment_date">
                            <span class="d-flex align-items-center">
                                <span>
                                    Ngày dự thu
                                </span>
                                <span>
                                    <span>
                                        <span
                                            class="material-symbols-rounded ms-2 sort-icon {{ $sortColumn == 'estimated_payment_date' ? '' : 'text-muted' }}">
                                            sort
                                        </span>
                                    </span>
                                </span>
                        ...
                        <th list-action="sort" sort-by="status"
                            ...
                            class="min-w-125px text-left fs-8" data-column="status">
                            <span class="d-flex align-items-center">
                                <span>
                                    Trạng thái
                                </span>
                ...
                <tbody class="text-gray-600">
                    @foreach ($account_kpi_notes as $account_kpi_note)
                        <tr list-control="item">
                            <td class="text-center ps-1">
                                <div class="form-check form-check-sm form-check-custom">
                                    <input data-item-id="{{ $account_kpi_note->id }}" list-action="check-item"
                                        class="form-check-input" type="checkbox" value="1" />
                                </div>
                            </td>

                            <td class="text-left" data-filter="mastercard" data-column="contact_id">
                                {{ $account_kpi_note->contact->code }}
                            </td>

                            <td class="text-left" data-filter="mastercard" data-column="contact_id">
                                {{ $account_kpi_note->contact->name }}
                            </td>

                            <td class="text-left" data-filter="mastercard" data-column="account_id">
                                {{ $account_kpi_note->account->code ?? '' }}
                            </td>

                            <td class="text-left" data-filter="mastercard" data-column="account_id">
                                {{ $account_kpi_note->account->name ?? ''}}
                            </td>

                            <td class="text-left" data-filter="mastercard" data-column="service_type">
                                {{ trans('messages.service_type.' . $account_kpi_note->service_type) }}
                            </td>

                            <td class="text-left" data-filter="mastercard" data-column="subject_id">
                                @if ($account_kpi_note->service_type == config('constants.SERVICE_TYPE_EDU'))
                                    Môn: {{ $account_kpi_note->subject ? $account_kpi_note->subject->name : '--' }}
                                @elseif ($account_kpi_note->service_type == config('constants.SERVICE_TYPE_EXTRACURRICULAR'))
                                    Loại: {{ $account_kpi_note->extracurricular_type }}
                                @endif
                            </td>

                            <td class="text-left" data-filter="mastercard" data-column="note">
                                {!! $account_kpi_note->note !!}
                            </td>


                            <td class="text-left" data-filter="mastercard" data-column="amount">
                                {!! number_format($account_kpi_note->amount) !!}đ
                            </td>


                            <td data-filter="mastercard" data-column="estimated_payment_date">
                                {{ \Carbon\Carbon::parse($account_kpi_note->estimated_payment_date)->format('d/m/Y') }}
                            </td>
                            <td data-filter="mastercard" data-column="status">
                                {{ $account_kpi_note->status ? trans('messages.account_kpi_note.status.' . $account_kpi_note->status) : '--' }}
                            ...
                            <td class="text-left">
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                    ...
                                    <div class="menu-item px-3">
                                        <a row-action="update"
                                            href="{{ action(
                                                [App\Http\Controllers\AccountKpiNoteController::class, 'edit'],
                                                [
                                                    'id' => $account_kpi_note->id,
                                                ],
                                            ) }}"
                                    ...
                                    <div class="menu-item px-3">
                                        <a row-action="delete"
                                            href="{{ action(
                                                [App\Http\Controllers\AccountKpiNoteController::class, 'destroy'],
                                                [
                                                    'id' => $account_kpi_note->id,
                                                ],
                                            ) }}"
    ...
    @else
        <div class="py-15">
            ...
            <p class="text-center d-none">
                <a class="btn btn-outline btn-outline-default" id="">
                    <span class="d-flex align-items-center">
                        <span class="material-symbols-rounded me-2">
                            add
                        </span>
                        <span>Thêm dự thu mới</span>
                    </span>
                </a>
            </p>
        </div>
