                ...
                @foreach ($calendar->getEvents() as $key => $event)
                    @switch($event[25])
                        ...
                        @case('dashboard')
                            ...
                            @break
                        @case('freetime')
                            @include('helpers.calendar.sections.free_time_section', [
                                'event' => $event,
                                'date' => $date,
                                'key' => $key
                            ])
...
<script script-control="calendar-script">
    $(() => {
        // $('[data-control="edit-event"], [data-control="delete-event"]').each(function(key, item) {
        ...
        // $('[data-item="event"]').on('click', function(e) {
        //     if ($(e.target).data('control') !== 'edit-event' && $(e.target).data('control') !== 'delete-event') {
        //         $(this).popover('toggle');
        //     }
        // });
        ...
        /**
         * below code is using in case tooltip
         */
        ...
        // ****************** above code using to sections what have pop class ******************
    ...
    var EventBox = class {
        ...
        clickAddEventButtonHandle(studyDate) {
            ...
            var isAbroad = "{{ $isAbroad }}"

            if (!!subjectId == false && !!isAbroad == true) {
                ASTool.alert({
            ...
            const url = "{!! action('App\Http\Controllers\CalendarController@addEventInCalendar') !!}";
