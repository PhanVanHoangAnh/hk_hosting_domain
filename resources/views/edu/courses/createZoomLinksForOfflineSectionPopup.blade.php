...
@section('title')
Tạo link zoom học online cho buổi học
@endsection
        ...
        <div class="pe-7 py-5 px-lg-17">
            <div class="row mb-7">
                <div class="col-md-12">
                    <div class="fv-row mb-7">
                        <label class="fs-6 fw-semibold mb-2">
                            <span class="" style="filter: blur(0.5px);">Link mở lớp đã tạo</span>
                        </label>
                        <div class="form-outline">
                            <input type="text" style="border-radius: 12px; filter: blur(0.5px); font-weight: 50; cursor: not-allowed; opacity: 0.6;" class="border-0 form-control {{ $section->zoom_start_link ? '' : 'fst-italic' }}" readonly value="{{ $section->zoom_start_link ? $section->zoom_start_link : 'Chưa có link' }}">
                ...
                <div class="col-md-12">
                    <div class="fv-row mb-7">
                        <label class="fs-6 fw-semibold mb-2">
                            <span class="" style="filter: blur(0.5px);">Link tham gia đã tạo</span>
                        </label>
                        <div class="form-outline">
                            <input type="text" style="border-radius: 12px; filter: blur(0.5px); font-weight: 50; cursor: not-allowed; opacity: 0.6;" class="border-0 form-control {{ $section->zoom_join_link ? '' : 'fst-italic' }}" readonly value="{{ $section->zoom_join_link ? $section->zoom_join_link : 'Chưa có link' }}">
                            <x-input-error :messages="$errors->get('contact_id')" class="mt-2"/>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-12">
                    <div class="fv-row mb-7">
                        <label class="fs-6 fw-semibold mb-2">
                            <span class="" style="filter: blur(0.5px);">Password đã có</span>
                        </label>
                        <div class="form-outline">
                            <input type="password" style="border-radius: 12px; filter: blur(0.5px); font-weight: 50; cursor: not-allowed; opacity: 0.6;" class="border-0 form-control {{ $section->zoom_password ? '' : 'fst-italic' }}" readonly value="{{ $section->zoom_password }}">
                            <x-input-error :messages="$errors->get('contact_id')" class="mt-2"/>
                        </div>
                    </div>
                </div>
            ...
            <div class="row mt-15">
                @if ($section->course->study_method == \App\Models\Course::STUDY_METHOD_OFFLINE)
                    @php
                    ...
                    <div class="row mb-4 position-relative" form-control="zoom" form-uniq-id="{{ $zoomFormUniqId }}">
                        <script>
                            ...
                            var ZoomManager = class {
                                ...
                                toggle() {
                                    let switchLabelContent = "Nhập thông tin Zoom đã có từ trước";
    ...
    <script>
        ...
        var UpdateZoomLinksManager = (function() {
            ...
            var submit = function() {
                ...
                var url = "{{ action('App\Http\Controllers\Edu\SectionController@createZoomLinksForOfflineSection', ['id' => 'PLACEHOLDER']) }}";
