<h3 class="mb-10">Thông tin lớp học</h3>
<input type="hidden" name="module" value="{{ \App\Models\Course::MODULE_EDU }}">
...
<div id="dataFillForm">
    ...
    <div class="row mb-4" data-container="subject-level">
        <script>
            var subjectSelector, levelSelector;
            ...
            var SubjectSelector = class {
                ...
                getSelector() {
                    return this.getContainer().find('[data-action="subject-select"]');
                }

                loadLevelOptionsBySubject(subjectId) {
                    let rawUrl = "{{ action([App\Http\Controllers\SubjectController::class, 'getLevelsBySubject'], ['subjectId' => 'PLACEHOLDER']) }}";
                    const finalUrl = rawUrl.replace('PLACEHOLDER', subjectId); 

                    $.ajax({
                        url: finalUrl,
                        data: {
                            levelSelected: "{{ isset($course) ? $course->level : '' }}",
                            _token: "{{ csrf_token() }}"
                        },
                        method: 'POST',
                    }).done(response => {
                        levelSelector.updateOption(response);
                    }).fail(response => {
                        throw new Error("Get level by subject fail!");
                    })
                }
                ...
                changeSubject() {
                    if (this.getSelector().select2('val')) {
                        if (this.getSelector().val()) {
                            this.loadLevelOptionsBySubject(this.getSelector().val());
                        }
                    }
                }
            ...
            var LevelSelector = class {
                ...
                getSelector() {
                    return this.getContainer().find('[data-action="level-selector"]');
                }
                
                updateOption(option) {
                    this.getSelector().html(option);
                }
            }
        </script>
        
        @includeIf('edu.courses._subjects_manager')

        <div class="col-lg-6 col-xl-3 col-md-6 col-sm-12 col-12 mb-4">
            <div class="form-outline" data-container="level-select">
                <label class="fs-6 fw-semibold mb-2" for="level-select">Trình độ</label>
                <select data-action="level-selector" id="level-select" class="form-select form-control {{ $action === 'edit' && $course->hasFinished() ? 'pe-none bg-secondary' : '' }}" name="level" data-control="select2" data-dropdown-parent="#dataFillForm" data-placeholder="Chọn trình độ">
                ...
                <script>
                    $(() => {
                        levelSelector = new LevelSelector({
                            container: () => {
                                return $('[data-container="level-select"]');
                            }
                        })
                    })
                </script>
        ...
        <div class="col-lg-6 col-xl-3 col-md-6 col-sm-12 col-12 mb-4">
            <div class="form-outline">
                <label class="required fs-6 fw-semibold mb-2" for="study-method">Hình thức học</label>
                <select id="study-method" class="form-select form-control {{ $action === 'edit' && $course->hasFinished() ? 'pe-none bg-secondary' : '' }}" data-action="study-method-select"
                    data-control="select2" placeholder="Chọn hình thức học..." name="study_method">
        ...
        <div class="col-lg-6 col-xl-3 col-md-6 col-sm-12 col-12 mb-4">
            <div class="form-outline">
                ...
                <select id="class-type-select" class="form-select form-control {{ $action === 'edit' && $course->hasFinished() ? 'pe-none bg-secondary' : '' }}" name="class_type" data-control="select2" data-dropdown-parent="#dataFillForm" data-placeholder="Chọn loại hình lớp" data-allow-clear="true">
        ...
        <div class="col-lg-6 col-xl-3 col-md-6 col-sm-12 col-12 mb-4">
            <div class="form-outline">
                <label for="start-time-input" class="required fs-6 fw-semibold mb-2">Thời gian bắt đầu học</label>
                <div data-control="date-with-clear-button" class="d-flex align-items-center date-with-clear-button">
                    <input id="start-time-input" data-control="input" {{ $action === 'edit' && $course->hasFinished() ? 'readonly' : '' }} value="{{ isset($course->start_at) ? \Carbon\Carbon::parse($course->start_at)->format('Y-m-d') : '' }}" name="start_at" placeholder="=asas" type="date" class="form-control" placeholder=""/>
                    <span data-control="clear" class="material-symbols-rounded clear-button" style="display:none;">close</span>
                </div>
                <x-input-error :messages="$errors->get('start_at')" class="mt-2"/>
            </div>
        </div>
        <div class="col-lg-9 col-xl-9 col-md-9 col-sm-12 col-12 mb-4 d-none" form-control="location">
            @include('edu.courses._location')
        </div>
        ...
        {{-- ZOOM --}}
        <div class="row d-none mb-4 position-relative" form-control="zoom" form-uniq-id="{{ $zoomFormUniqId }}">
            <script>
                $(() => {
            ...
            <div class="form-check form-switch ms-3 mb-5" style="--bs-form-switch-width:60px; --bs-form-switch-height:24px">
                <input data-action="zoom-type-switch" class="form-check-input" type="checkbox" role="switch" id="zoom_fill_type_switch" name="zoom_switch"
                       {{ isset($formReturn['zoom_switch']) && $formReturn['zoom_switch'] === "on" ? 'checked' : '' }}
                ...
                <script>
                    var ZoomAutoManager = class {
                        ...
                        reloadAvailableOptions(availableZoomUserIds) {
                            ...
                            // Get current zoom account option selected
                            const zoomUserIdSelected = _this.getCurrentZoomUserId();
                            ...
                            $.ajax({
                                ...
                                const availableZoomUserIds = [];

                                if (zoomUserIdSelected) {
                                    const options = _this.getZoomUserIdSelect().find('option');
                                
                                    options.each((key, item) => {
                                        const optionValue = $(item).attr('value'); 
                                        if (optionValue && optionValue !== "") {
                                            availableZoomUserIds.push(optionValue);
                                        }
                                    })
                                
                                    if (jQuery.inArray(zoomUserIdSelected, availableZoomUserIds) != -1) {
                                        _this.getZoomUserIdSelect().val(zoomUserIdSelected);
                                    } else {
                                        _this.getZoomUserIdSelect().val("");
                                    }
                                }
            ...
            <div form-control="zoom-past-value">
                ...
                <div class="col-lg-11 col-xl-11 col-md-11 col-sm-11 col-11 mb-6">
                    <label class="fs-6 mb-3" for="zoom_start_link">Link mở phòng học (cho chủ phòng)</label>
                    <input id="zoom_start_link" type="text" class="form-control" placeholder="Nhập link mở phòng..." name="zoom_start_link"
                ...
                <!-- Join link form -->
                <div class="col-lg-11 col-xl-11 col-md-11 col-sm-11 col-11 mb-6">
                    <label class="fs-6 mb-3" for="zoom_join_link">Link tham gia (cho các học viên)</label>
                    <input id="zoom_join_link" type="text" class="form-control" placeholder="Nhập link tham gia..." name="zoom_join_link" 
                ...
                <div class="col-lg-2 col-xl-2 col-md-2 col-sm-2 col-2 mb-6">
                    <label class="fs-6 mb-3" for="zoom_password">Mật khẩu</label>
                    <input id="zoom_password" type="password" placeholder="Nhập mật khẩu phòng" class="form-control text-center" name="zoom_password" autocomplete="on"
...
<div class="row mt-10 p-0">
    <div class="col-lg-12 col-xl-5 col-md-12 col-sm-12 col-12 pe-6 {{ $action === 'edit' && $course->hasFinished() ? 'd-none' : '' }}">
    ...
    <div class="col-lg-12 {{ $action === 'edit' && $course->hasFinished() ? 'col-xl-12' : 'col-xl-7' }} col-md-12 col-sm-12 col-12 mt-14">
        {{-- Tabs menu --}}
...
<div class="form-outline d-flex justify-content-center">
    <div class="mb-10">
        ...
        <a href="{{ action('App\Http\Controllers\Edu\CourseController@index') }}" id="cancelAddCourseBtn"
...
<script>
    ...
    var WeekScheduleTool = class {
         ...
         * @return void
         */
        applyToCalendar(handleWhenApplySuccess, handleWhenApplyFail) {
            const currWeekScheduleEvents = this.convertSchedulesToEvents();
            const totalHours = addCourseManager.getTotalHoursInputValue() ? parseFloat(addCourseManager.getTotalHoursInputValue()) : 0;
            const testHours = addCourseManager.getTestHoursInputValue() ? parseFloat(addCourseManager.getTestHoursInputValue()) : 0;
            const inputTotalHours = totalHours + testHours;

            let totalMinutes = 0;
            
            currWeekScheduleEvents.forEach(item => {
            ...
            /* Call ajax request to the server to check if the selected Zoom account has any time coflicts with the newly created sections
                Check condition before request ajax to server:
                - Study method must be online (It's not neccessary but go ahead and check explicitly)
            */
            const onlineMethod = "{!! \App\Models\Course::STUDY_METHOD_ONLINE !!}";
    
            if (addCourseManager.getStudyMethodSelectValue().toLowerCase() === onlineMethod.toLowerCase()) {
                // Pass all precheck
                // Call ajax to get available zoom user ids
                $.ajax({
                    url: "{{ action([App\Http\Controllers\ZoomMeetingController::class, 'getAvailableZoomUserIdsBySections']) }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        sections: currWeekScheduleEvents
                    }
                }).done(res => {
                    if (zoomManager.getZoomAutoManager()) {
                        zoomManager.getZoomAutoManager().reloadAvailableOptions(res.availableZoomUserIds);
                    }

                    // Apply new events to calendar data, the calendar will update new UI and events automatically
                    calendar.updateEvents(currWeekScheduleEvents);
                    timeAutoCaculatForm.run();
                    handleWhenApplySuccess();
                }).fail(res => {
                    handleWhenApplyFail();
                })
            } else {
        ...
        eventsAfterRender() {
            ...
            _this.getAddScheduleButtons().forEach(button => {
                $(button).on('click', e => {
                    ...
                    const url = "{!! action('App\Http\Controllers\Edu\CourseController@addSchedule') !!}";
            ...
            _this.getEditScheduleBtns().forEach(button => {
                $(button).on('click', e => {
                    ...
                    const url = "{{ action('App\Http\Controllers\Edu\CourseController@editSchedule') }}";
                    const id = button.getAttribute('data-id-control');
            ...
            if ($(addCourseManager.getContainer())[0]) {
                ...
                initJs($(addCourseManager.getContainer()).find('#weekScheduleTool')[0])
        ...
        events() {
            ...
            // Apply week schedule to calendar
            $(_this.getApplyBtn()).on('click', e => {
                ...
                try {
                    ...
                    var handleWhenApplySuccess = function() {
                        ASTool.alert({
                    ...
                    var handleWhenApplyFail = function() {
                        ASTool.alert({
                            message: "Cấu hình thời khóa biểu không hợp lý dẫn đến tạo ra các buổi học với tổng thời gian học mỗi buổi không phù hợp với cấu hình tổng giờ học và giờ kiểm tra ở trên!",
                            icon: 'warning'
                        })
                    ...
                    _this.applyToCalendar(handleWhenApplySuccess, handleWhenApplyFail);
                } catch (error) {
                    ASTool.alert({
                        message: "Đã xảy ra lỗi trong quá trình áp dụng thời khóa biểu!",
                        icon: 'error'
                    });
    ...
    var AddCourseManager = class {
        ...
        queryHomeRoomsByArea(area) {
            let url = "{{ action([App\Http\Controllers\Edu\StaffController::class, 'getHomeRoomByArea'], ['area' => 'PLACEHOLDER']) }}";
        ...
        getTestHoursInputValue() {
            return this.getTestHoursInput().value;
        };
        ...
        submit() {
            ...
            $.ajax({
                ...
                ASTool.alert({
                    ...
                    ok: () => {
                        this.addLoadingEffect();
                        window.location.href = "{{ action('App\Http\Controllers\Edu\CourseController@index') }}";
                    }
                });
            }).fail(response => {
                this.removeSubmitEffect();
        ...
        changeStudyMethodHandle() {
            ...
            if (studyMethod === "" || !studyMethod) {
                ...
                if (studyMethod === onlineMethod) {
                    ...
                    homerooms = JSON.parse(JSON.stringify({!! \App\Models\Teacher::homeRooms()->get() !!}));
        ...
        events() {
            ...
            $(_this.getTestHoursInput()).on('change', function(e) {
                e.preventDefault();
                timeAutoCaculatForm.loadTimesInformation();
            });
    ...
    var TimeAutoCaculatForm = class {
        ...
        getTotalHoursInputValue() {
            const totalHours = addCourseManager.getTotalHoursInputValue() ? parseFloat(addCourseManager.getTotalHoursInputValue()) : 0;
            const testHours = addCourseManager.getTestHoursInputValue() ? parseFloat(addCourseManager.getTestHoursInputValue()) : 0;
            return totalHours + testHours;
        };

        getTestHoursInputValue() {
            return addCourseManager.getTestHoursInputValue();
        };
