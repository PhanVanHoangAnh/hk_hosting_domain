...
<div>
    ...
    <div class="card-header border-0" id="addCourseContainer">
        <form id="courseForm" action="{{ action([App\Http\Controllers\Edu\CourseController::class, 'updateCalendar'], ['id' => $course->id]) }}" method="put">
            @csrf
            @include('edu.courses._form_edit_calendar')
        </form>
    </div>
</div>
@endsection