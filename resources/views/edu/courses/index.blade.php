        ...
        <div class="d-flex align-items-center py-1">
            @if (Auth::user()->can('create', App\Models\Contact::class))
                ...
                <a href="{{ action([App\Http\Controllers\Edu\CourseController::class, 'add']) }}"
                    class="btn btn-flex btn-sm btn-primary fw-bold border-0 fs-6 h-45px" id="buttonCreateContact">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                ...
                                <div class="menu-item px-3">
                                    <a row-action="delete-all"
                                        href="{{ action([App\Http\Controllers\Edu\CourseController::class, 'deleteAll']) }}"
            ...
            <div class="card-header border-0 p-4 pb-0 d-none list-filter-box" list-action="filter-action-box">
                ...
                <div class="card-toolbar w-100 mb-0" list-action="tool-action-box">
                    ...
                    <div class="row w-100">
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-6 col-6 mb-5">
                            {{-- @if (Auth::user()->can('changeBranch', User::class) || !Auth::user()->account->teacher)  --}}
                            <!--begin::Label-->
                            ...
                            <select list-action="teacher-filter-select" class="form-select filter-select"
                                ...
                                @php
                                    $teacherIds = App\Models\Course::edu()->pluck('teacher_id')->unique();
                                @endphp
                                @foreach ($teacherIds as $teacherId)
                                    <option value="{{ isset($teacherId) ?  $teacherId : '' }}">{{ isset($teacherId) ? App\Models\Teacher::find($teacherId)->name : '' }}</option>
                                @endforeach
                            </select>
                            {{-- @endif --}}
                        </div>
                    ...
                    <div class="row w-100">
                        ...
                        {{-- <div class="col-md-3 mb-5">
                            ...
                            <select list-action="branch" class="form-select filter-select" data-control="select2"
                                data-close-on-select="false" data-placeholder="Chọn chi nhánh" data-allow-clear="true"
                                multiple="multiple">
                        ...
                        <div class="col-md-3 mb-5">
                            <label class="form-label fw-semibold">Địa điểm học</label>

                            <select list-action="subject-filter-select" class="form-select filter-select"
                                data-control="select2" data-kt-customer-table-filter="month"
                                data-placeholder="Chọn địa điểm học" data-allow-clear="true" multiple="multiple">
                                @foreach (App\Models\Subject::getAllSubjectsInUnique() as $subject)
                                    <option value="{{ $subject->id }}">{{ $subject->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-5">
                            ...
                            <div class="col-12" id="locationUniqId_{{ $locationUniqId }}">
                                <div class="row">
                                    <div class="col-6">
                                        <div class="form-outline">
                                            <label class="fs-6 fw-semibold" for="branch-select">Chọn chi nhánh</label>
                                            <select list-action="branch" id="branch-select" class="form-select form-control filter-select" data-control="select2"
                                                data-dropdown-parent="#locationUniqId_{{ $locationUniqId }}"
                                                data-placeholder="Chọn chi nhánh">
                                                <option value="">Chọn chi nhánh</option>
                                                @foreach (App\Models\TrainingLocation::getBranchs() as $branch)
                                                    <option value="{{ $branch }}"
                                                        {{ \App\Library\Branch::getCurrentBranch() === $branch
                                                            ? 'selected'
                                                            : '' }}>
                                    ...
                                    <div class="col-6">
                                        <div class="form-outline">
                                            <label class="fs-6 fw-semibold" for="location-select">Chọn địa điểm</label>
                                            <select list-action="locations" id="location-select" class="form-select form-control filter-select" name="training_location_id"
                                                data-dropdown-parent="#locationUniqId_{{ $locationUniqId }}"
                                                data-control="select2" data-placeholder="Chọn địa điểm" multiple="multiple">
                                            </select>
                                        </div>
    ...
    <script>
        ...
        let CoursesList = function() {
            ...
            return {
                init: () => {
                    list = new DataList({
                        url: "{{ action('\App\Http\Controllers\Edu\CourseController@list') }}",
        ...
        const filterData = function() {
            ...
            const getLocationsValue = () => {
                locations = Array.from(document.querySelector('[list-action="locations"]')
                        .selectedOptions)
                    .map(option => option.value);
            };
            ...
            const getTeacherValues = () => {
                const selectElement = document.querySelector('[list-action="teacher-filter-select"]');
                if (!selectElement) {
                    return; 
                }

                teachers = Array.from(selectElement.selectedOptions)
                    .map(option => option.value);
            ...
            return {
                init: () => {
                    $('.filter-select').on('change', () => {
                        ...
                        CoursesList.getList().setLocations(locations);
                    ...
                    $('[list-action="export-excel"]').on('click', function() {
                        ...
                        const query = {
                            ...
                            created_at_from: $('[name="created_at_from"]').val(),
                            created_at_to: $('[name="created_at_to"]').val(),
                            updated_at_from: $('[name="updated_at_from"]').val(),
                            updated_at_to: $('[name="updated_at_to"]').val(),
                            start_at_from: $('[name="started_time_from"]').val(),
                            start_at_to: $('[name="started_time_to"]').val(),
                            end_at_from: $('[name="end_time_from"]').val(),
                            end_at_to: $('[name="end_time_to"]').val(),
                            status: '{{ request()->status ?? 'all' }}'
                        };

                        $.ajax({
                            url: "{{ action('\App\Http\Controllers\Edu\CourseController@export') }}",
                            type: "GET",
                            data: query,
                            xhrFields: {
                                responseType: 'blob' 
        ...
        let DataList = class {
            ...
            initContentEvents() {
                ...
                if (this.getNumberOfItems()) {
                     ...
                     * COPY COURSE
                     */
                    // this.getCopyClassBtn().forEach(button => {
                    //     button.addEventListener('click', function(e) {
                    //         e.preventDefault();
                    //         ASTool.addPageLoadingEffect();

                    //         const courseId = $(e.target).attr('data-course-id');
                    ...
                    this.getDeleteAllItemsBtn().addEventListener('click', function() {
                        ...
                        ASTool.confirm({
                            ...
                            ok: function() {
                                ...
                                $.ajax({
                                    url: "{{ action('\App\Http\Controllers\Edu\CourseController@deleteAll') }}",
                                    method: "DELETE",
                                    data: {
                                        _token: "{{ csrf_token() }}",
                                        items: items
