...
@php
    ...
    $action = action([App\Http\Controllers\Edu\CourseController::class, 'createScheduleItem']);

    if(isset($weekEventEdit)) $action = action([App\Http\Controllers\Edu\CourseController::class, 'editWeekScheduleItem']);
    if(isset($isAddEventInCalendar)) $action = action([App\Http\Controllers\CalendarController::class, 'createEventInCalendar']);
    if(isset($weekEventEditInCalendar)) $action = action([App\Http\Controllers\CalendarController::class, 'updateEventInCalendar']);
@endphp
...
@section('content')
           ...
           <div class="p-10" id={{ $popupUnidId }}>
            <div class="row mb-8">
                ...
                <div class="card p-7 mb-4">
                    ...
                    @if (isset($zoomUsers))
                    {{-- Update Zoom user --}}
                    <!-- Optional, only select new user if an update is needed -->
                    <div class="row mt-4 d-none">
                        <label class="fs-6 fw-semibold mb-2" for="course-teacher-select">Cập nhật tài khoản Zoom</label>
                        <select class="form-select form-control" name="zoom_user_id" data-dropdown-parent="#{{ $uniqId }}"
                            ...
                            @foreach ($zoomUsers as $user)
                                ...
                                {{ isset($course->zoom_user_id) && $course->zoom_user_id == $user['user_id'] ? 'selected' : (isset($courseCopy->zoom_user_id) && $courseCopy->zoom_user_id == $user['user_id'] ? 'selected' : '') }}>
                                    {{ $user['email'] . " (" . $user['display_name'] . " - " . $user['dept'] . ")" }}
                                </option>
                            @endforeach
                        </select>
                    </div>
    ...
    <script>
                ...
                init: function() {
                    document.querySelector('[data-action="suggest-teacher"]').addEventListener('click', e => {
                        ...
                        suggestTeacherPopup.updateUrl(
                            "{{ action('\App\Http\Controllers\Edu\CourseController@suggestTeacher') }}"
                        );
                    });
                }
            }
        ...
        var AddScheduleManager = class {
            ...
            getZoomUserId() {
                return $(this.getContainer()).find('[name="zoom_user_id"]').val();
            }
            ...
            getDataEditedInCalendar() {
                ...
                return {
                    ...
                    // Remember this zoom_user_id is optional(not required)
                    // if not selected, the value is ""
                    zoom_user_id: this.getZoomUserId(),
                };
            };
            ...
            handleCheckVnTeacher() {
                if(this.getVnTeacherCheckBox().checked) {
                    ...
                    if (this.getVnTeacherFromInput().value == "") {
                        this.getVnTeacherFromInput().value = this.getStartAtValue();
                    }

                    if (this.getVnTeacherToInput().value == "") {
                        this.getVnTeacherToInput().value = this.getEndAtValue();
                    }

                    this.getForeignTeacherCheckBox().checked = false;
            ...
            handleCheckForeignTeacher() {
                if(this.getForeignTeacherCheckBox().checked) {
                    ...
                    if (this.getForeignTeacherFromInput().value == "") {
                        this.getForeignTeacherFromInput().value = this.getStartAtValue();
                    }

                    if (this.getForeignTeacherToInput().value == "") {
                        this.getForeignTeacherToInput().value = this.getEndAtValue();
                    }

                    this.getVnTeacherCheckBox().checked = false;
                    this.disableAllVnTeacherInputs();
                    this.disableAllVnTeacherSelects();
                    this.highLineVnTeacherRow();
                    this.getVnTeacherFromInput().value = null;
            ...
            events() {
                ...
                $(this.getAssistantFromInput()).on('change', function(e) {
                    e.preventDefault();
                    _this.displayAssistantDuration();
                });

                $(this.getAssistantToInput()).on('change', function(e) {
                    e.preventDefault();
                    _this.displayAssistantDuration();
                });
