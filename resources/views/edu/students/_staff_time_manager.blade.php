...
<div class="row" id="{{ $staffTimesManagerContainerUniqId }}">
    <div class="col-lg-12 col-md-12 col-sm-12 col-12 mb-8 mt-8">
        @php
            $newOrderItemStaffTimesManagerUniqId = 'S' . uniqid();
        @endphp
    
        <div id="{{ $newOrderItemStaffTimesManagerUniqId }}" form-container="newOrderItemStaffTimesManagerUniqId">
            {{-- Discount percent --}}
            <div class="mb-2">
                Giảm giá: <span class="fw-bold" data-form="discountPercent_{{ $newOrderItemStaffTimesManagerUniqId }}"></span> %
                <input type="hidden" name="discount_percent" value="">
                <script>
                    var discountPercent = 0;
        
                    document.querySelector('[data-form="discountPercent_{{ $newOrderItemStaffTimesManagerUniqId }}"]').innerText = discountPercent;
        ...
        <script>    
            ...
            var StaffTimesManager = class {
                ...
                getDiscountPercentInput() {
                    return this.container().querySelector('[data-form="discountPercent_{{ $newOrderItemStaffTimesManagerUniqId }}"]');
                }
    ...
    <script>
        $(() => {
            ...
            //             container: () => {
            //                 return document.querySelector('#{{ $newOrderItemStaffTimesManagerUniqId }}')
            //             }
            //         }),
            //     })
            // }, 1);

            new StaffTimesManager({
                container: () => {
                    return document.querySelector('#{{ $newOrderItemStaffTimesManagerUniqId }}')
                }
            })
        ...
        var RevenueToolForm = class {
            ...
            load() {
                const data = {
                    ...
                    revenueDistributions: "{{ isset($revenueData) ? $revenueData : json_encode( isset($orderItem) && $orderItem->revenueDistributions->count() > 0 ? \App\Helpers\RevenueDistribution::processRevenueDistributeList($orderItem->revenueDistributions, 2000) : \App\Helpers\RevenueDistribution::getRevenueDataWithDefault(2000) ) }}",
                }   
