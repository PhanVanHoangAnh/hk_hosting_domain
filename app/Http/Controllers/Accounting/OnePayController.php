...
class OnePayController extends Controller
{
    public function onepayPayment($id, $amount){
        ...
        $url  = url('accounting/onepay/process');
    ...
    {
        $order =  Order::find($id);
        return view('accounting.orders.onepay.createReceipt', [
            'order' =>$order,
    ...
    public function showLink(Request $request)
    {
       ...
       $redirectUrl = $this->onepayPayment($id,$amount );
        return view('accounting.orders.onepay.showLink', [
            'order' =>$order,
...
////Trả góp
    ...
    public function onepayPaymentInstallment($id, $amount){
        $order = Order::find($id);
        // tao 1 phieu thu voi status == pending
        $paymentRecord = $order->createPendingPaymentRecord($amount);
        // Xu ly onepay de tao link payment
    ...
    public function processInstallment(Request $request)
    {
    ...
    public function checkRequestInstallment()
    {
        $url = \Request::getRequestUri();
        $parts = parse_url($url);
        $queriesString = $parts['query'];
    ...
    public function createReceiptInstallment(Request $request, $id)
    {
        $order =  Order::find($id);
        return view('accounting.orders.onepay.createReceiptInstallment', [
            'order' =>$order,
        
        ]);
    }
    public function showLinkInstallment(Request $request)
    {
       $id = $request->id;
       $amount = $request->amount;
       $order = Order::find($id);
       ...
       $redirectUrl = $this->onepayPayment($id,$amount );
        return view('accounting.orders.onepay.showLinkInstallment', [
            'order' =>$order,
            'redirectUrl'=>$redirectUrl,
        ]);
    }
