...
class Util
{
    ...
    public function getRedirectUrl($apiUrl)
    {
        // Khởi tạo cURL handle
        $curl = curl_init($apiUrl);
    
        // Đặt cờ để cURL trả về phản hồi thay vì hiển thị ra màn hình
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
    
        // Thực hiện yêu cầu HTTP và lấy phản hồi
        $response = curl_exec($curl);
    
        // Lấy thông tin về yêu cầu HTTP
        $info = curl_getinfo($curl);
    
        // Lấy URL chuyển hướng từ thông tin yêu cầu
