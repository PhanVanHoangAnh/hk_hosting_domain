...
class ShoolBirthday30KUpdater
{
    ...
    public function getRowLog($row)
    {
        ...
        return "name: #{$data['name']}, phone: #{$data['phone']}, MotherPhone: #{$data['MotherPhone']}, FatherPhone: #{$data['FatherPhone']}, school: #{$data['school']}, birthday: #{$data['birthday']}";
    }
    ...
    public function processData($data, $index)
    {
        ...
        if (empty($data['phone']) && empty($data['MotherPhone']) && empty($data['FatherPhone'])){
            // logging
            // $this->logInfo("PHONE + MOTHER + FATHER MISSING => ignore. " . $this->getRowLog($data));
        ...
        //     // logging
        //     $this->logInfo("PHONE & NAME MISSING=> ignore. " . $this->getRowLog($data));

        //     return;
        // }

        // Create/Update contact
        list($contact, $isNew) = $this->createUpdateContacts($data);
    ...
    public function createUpdateContacts($data)
    {
        // Mother
        $mother = null;
        ...
        $qq = Contact::active()
            ->relatedContactsByPhoneMotherPhoneFatherPhone($data['phone_raw'], $data['MotherPhone'], $data['FatherPhone']);
        $contacts = $qq->get();

        // validation
        if ($qq->count() > 3) {
            var_dump($data['phone_raw']);
            var_dump($data['MotherPhone']);
            var_dump($data['FatherPhone']);
            die();
        }
        ...
        foreach ($contacts as $contact) {
            // logging
            $this->logInfo("EXIST CONTACT [{$qq->count()}] => get contact " . $this->getRowLog($data));
            ...
            foreach($contact->contactRequests as $contactRequest) {
    ...
    public function createUpdateContactRequest($contact, $data)
    {
        ...
        if (!$contactRequest) {
            // logging
            $this->logInfo("NEW REQUEST ==> bỏ qua : " . $this->getRowLog($data));
            return;
        }
        ...
        // Nếu có request trong db rồi thì update
            ...
            if ($this->updateContactRequest) {
                // logging
                $this->logInfo("EXIST REQUEST ==> updated school, birthday : " . $this->getRowLog($data));
