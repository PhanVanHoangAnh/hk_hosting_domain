...
class GoogleSheetUpdater
{
    ...
    public function __construct($sheetId)
    {
        ...
        $this->datas = $this->service->readContactSyncSheet('Sheet1!A107853:Z107856');
        ...
        $this->datas = array_map(function($row) {
            ...
            try {
                $assigned_date = !isset($row[array_search('AssignedDate', $headers)]) ? '' : (!empty($row[array_search('AssignedDate', $headers)]) ?
                    Carbon::createFromFormat('d/m/y', $row[array_search('AssignedDate', $headers)])->startOfDay()
                    : null);
            } catch (\Throwable $e) {
            ...
            // try {
                if (isset($row[array_search('FormSubmitDate', $headers)]) && strlen($row[array_search('FormSubmitDate', $headers)]) >= 9) {
                    $google_form_submit_date = !isset($row[array_search('FormSubmitDate', $headers)]) ? '' : (!empty($row[array_search('FormSubmitDate', $headers)]) ?
                        Carbon::createFromFormat('m/d/Y', $row[array_search('FormSubmitDate', $headers)])->startOfDay()
                        : null);
                } else {
                    $google_form_submit_date = !isset($row[array_search('FormSubmitDate', $headers)]) ? '' : (!empty($row[array_search('FormSubmitDate', $headers)]) ?
                        Carbon::createFromFormat('d/m/y', $row[array_search('FormSubmitDate', $headers)])->startOfDay()
                        : null);
                }
                    
            // } catch (\Throwable $e) {
            //     echo $row[array_search('FormSubmitDate', $headers)] . $e->getMessage() . "\n";
