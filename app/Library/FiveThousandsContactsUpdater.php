...
class FiveThousandsContactsUpdater
{
    public $logger;
    public $sheetId;
    ...
    public function run($force = false)
    {
        ...
        try {
            ...
            $data = $service->readContactSyncSheet('Sheet1!A' . $lastImportedLine . ':AN300000');
    ...
    public function processDatas($headers, $rows)
    {
        ...
        $datas = array_map(function ($row) use ($headers) {
            ...
            return [
                ...
                'contact_request_id' => $row[array_search('SystemID', $headers)],
    ...
    {
        $contactRequest = \App\Models\ContactRequest::find($data['contact_request_id']);
        if ($this->log == 'console') {
            $this->logInfo("## Index {$index} -- ID : {$data['contact_request_id']} -- lead_status: {$data['lead_status']} Owner: {$data['contact_owner']} -- Note_Sales: {$data['note_sales']}");
        }

        if ($contactRequest->phone != $data['phone_raw'] || $contactRequest->demand != $data['demand']) {
            $this->logError("Phone $ Demand not match!!!!! {$contactRequest->phone} - {$data['phone_raw']}");
            die();
        }
