...
class GoogleSheetImporterNew
{
    ...
    public function processData($data, $index)
    {
        ...
        if (strpos($data['demand'], ';') !== false) {
            ...
            // không có demands nào từ chuỗi ==> demand rỗng
            } else {
                $data['demand'] = '';
                $this->createUpdateContactRequest($contact, $data);
            }
    ...
    public function createUpdateContact($data)
    {
        ...
        if (!$contact->google_form_submit_date) {
            $contact->google_form_submit_date = $data['google_form_submit_date'];
        }
        if (!$contact->latest_activity_date) {
    ...
    public function createUpdateContactRequest($contact, $data)
    {
        $contactRequest = \App\Models\ContactRequest::active()
            ->relatedContactsByPhoneMotherPhoneFatherPhoneAndDemandAndSubChannel($data['phone_raw'], $data['MotherPhone'], $data['FatherPhone'], $data['demand'], $data['sub_channel'])
            ->first();
        ...
        // Nếu có request trong db rồi thì update
            ...
            if ($this->updateContactRequest) {
                ...
                var_dump($data['lead_status']);
                var_dump(\App\Helpers\Functions::leadStatusMapping($data['lead_status']));
