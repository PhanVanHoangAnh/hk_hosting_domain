...
class SingleContactCleaner
{
    ...
    public function run()
    {
        ...
        $this->cleanupDuplicatedContact($contact);
        $this->cleanupContactRequestsByContact($contact);
    }
    ...
    // {
    //     // cleanup all phone is not valid
    //     // ContactRequest::where('import_id', '=', null)->where(\DB::raw('LENGTH(phone)'), '>', 30)->delete();
    //     // ContactRequest::where('import_id', '=', null)->where(\DB::raw('LENGTH(phone)'), '<', 6)->delete();
    ...
    public function cleanupContactRequestsByContact($contact)
    {
        // update phone
        // $contact->contactRequests()->update(['phone' => $contact->phone]);
    ...
    public function findRelatedContactRequests($contactRequest)
    {
        return $contactRequest->contact->contactRequests()
            ->relatedContactsByPhoneAndDemandAndSubChannel($contactRequest->phone, $contactRequest->demand, $contactRequest->sub_channel)
    ...
    public function removeDupRequests($contactRequest)
    {
        ...
        foreach ($dupRequests as $dupRequest) {
            ...
            if (!$contactRequest->note_sales && $dupRequest->note_sales) {
                $contactRequest->note_sales = $dupRequest->note_sales;
            }
    ...
    // {
    //     // cleanup all phone is not valid
    //     // Contact::where('import_id', '=', null)->where(\DB::raw('LENGTH(phone)'), '>', 30)->delete();
    //     // Contact::where('import_id', '=', null)->where(\DB::raw('LENGTH(phone)'), '<', 6)->delete();

    //     // // Contacts that have import_id
    //     // $contacts = Contact::where('import_id', '!=', null)->orderBy('updated_at', 'desc')->get();
    //     // foreach ($contacts as $index => $contact)
    //     // {
    //     //     // cleanup duplicates
    ...
    public function findRelatedContactsByPhoneOnly($contact)
    {
        return Contact::active()
            ->relatedContactsByPhone($contact->phone)
            ->where('import_id', '=', null)
            ->whereNot('id', $contact->id)
            ->get();
    }

    public function cleanupDuplicatedContact($contact)
    {
        // already removed
        ...
        foreach ($duplicatedContacts as $duplicatedContact) {
            ...
            if (!$contact->note_sales && $duplicatedContact->note_sales) {
                $contact->note_sales = $duplicatedContact->note_sales;
            }
